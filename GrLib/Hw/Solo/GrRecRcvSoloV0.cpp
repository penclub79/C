...
#define E_GrRecRcvSoloV0DfltPrefrmMs		6000

#define E_GrRecRcvSoloV0VssHeadCnt			2
#define E_GrRecRcvSoloV0VssTailCnt			8

//#define E_GrRecRcvSoloV0MaxSizeMode			
...
//--------------------------------------------------------------------
Cls_GrRecRcvSoloV0::Cls_GrRecRcvSoloV0( Cls_GrTaskSvr* A_TaskSvr, Cls_GrDevSolo* A_DevSolo, 
																	 Cls_GrDvrFsoMng* A_FsMng, BOOL8 A_IsUseVss ):
Cls_GrTaskCli()
{
	// local -------------------
		__u32		Tv_WkIdx;
	// code --------------------
		...
		m_TimeSecLast		=	0;
		m_TimeMsecLast	=	0;
		m_PrefrmTick		=	GrTimeMsToTick( E_GrRecRcvSoloV0DfltPrefrmMs );
		...
		if ( NULL == m_PtrRecQue )
		{
			DbgMsgPrint( "Cls_GrRecRcvSoloV0:: Solo object need run DevConfg!\n" );
		}
		else
		{
			// calculate buffer limite
			m_BufLimite	=	(m_PtrRecQue->BufSize >> 1) + (m_PtrRecQue->BufSize >> 2);
			DbgMsgPrint( "Cls_GrRecRcvSoloV0:: Preframe buffer limite = %d!\n", m_BufLimite );
		}

		//check header size
		if ( E_SoloVencIdxTblSize < sizeof(St_GrDvrAvHd) )
		{
			DbgMsgPrint( "Cls_GrRecRcvSoloV0:: video frame header bigger than p2m pad = %d !\n", sizeof(St_GrDvrAvHd) );
		}
		if ( 0 != (0x07 & sizeof(St_GrRecRcvSoloV0AdoPkt) ) )
		{
			DbgMsgPrint( "Cls_GrRecRcvSoloV0:: audio packet not aligned = %d !\n", sizeof(St_GrRecRcvSoloV0AdoPkt) );
		}

		//vss reserve queu
		for ( Tv_WkIdx=0;Tv_WkIdx < E_GrDvrMaxChCnt;Tv_WkIdx++ )
		{
			GrBufQueInit( (Ptr_GrBufQue)m_VssQue[Tv_WkIdx], sizeof(St_GrRecRcvSoloV0VssRsv), E_GrRecRcvVssRsvBufSize );
		}
...
//--------------------------------------------------------------------
Cls_GrRecRcvSoloV0::~Cls_GrRecRcvSoloV0()
{
...
//--------------------------------------------------------------------
BOOL8	Cls_GrRecRcvSoloV0::RtlBkgProc( void )
{
	// local -------------------
		BOOL8	Tv_Result;
	// code --------------------
...
//--------------------------------------------------------------------
void	Cls_GrRecRcvSoloV0::RtlTimer( __u32 A_Hndl )
{
	// local -------------------
		__u32	Tv_WkIdx;
		BOOL8	Tv_IsEnc;
	// code --------------------
		...
		if ( 0 == m_ObjDev->VencWatchdogCount() )
		{
			...
			if ( Tv_IsEnc )
			{
				...
				DbgMsgPrint( "Cls_GrRecRcvSoloV0::RtlTimer - capture freeze, reset capture!\n" );
			}
			else
			{
				DbgMsgPrint( "Cls_GrRecRcvSoloV0::RtlTimer - capture channel not exist.\n" );
			}
		}
...
#ifndef E_GrRecRcvSoloV0MaxSizeMode
		for ( Tv_WkIdx=0;Tv_WkIdx < m_VdoChCnt;Tv_WkIdx++ )
		{
...
//--------------------------------------------------------------------
void	Cls_GrRecRcvSoloV0::LcProcMtnPlugin( void* A_PtrFrm )
{
	// local -------------------
		__u32*	Tv_PtrHwHd;
		__u8		Tv_Res;
		__u8		Tv_Ch;
		__u8		Tv_Vop;
		St_GrRecRcvMtnCbMsg	Tv_MtnCbMsg;
		__u32		Tv_WkIdx;
	// code --------------------
		//process motion map
		if ( 0 != m_MtnMapSize )
...
//--------------------------------------------------------------------
BOOL8	Cls_GrRecRcvSoloV0::LcProcVdoFrame( void* A_PtrFrm, BOOL8 A_IsPreFlush )
{
	// local -------------------
		BOOL8		Tv_Result;
		__u32*	Tv_PtrHwHd;
		...
		Ptr_GrRecRcvSoloV0VssRsv	Tv_PtrVssItm;
	// code --------------------
		Tv_Result		=	TRUE;
		Tv_PtrHwHd	=	(__u32*)A_PtrFrm;
		//get event
		...
		if ( (E_GrDvrVdoResBad != Tv_Res) && ( (E_GrDvrVencGopTypeI == Tv_Vop) || m_VdoAbleTbl[Tv_Ch] ) )
		{
			...
			if ( (0 != Tv_Evt) || A_IsPreFlush )
			{
				...
				//make stream tail
				Tv_PktSize	=	E_GrDvrFsoStrmAlignMask & (Tv_CodeSize + sizeof(St_GrDvrAvHd) + E_GrDvrRecTailMarkSize + E_GrDvrFsoStrmAlignSize - 1 );
				...
				*Tv_PtrTail	=	E_GrDvrRecTailMarkId | Tv_PktSize;
				...
				{
					//check exist reserve
					Tv_PtrVssItm	=	(Ptr_GrRecRcvSoloV0VssRsv)GrBufQueGetOutPtr( (Ptr_GrBufQue)m_VssQue[Tv_Ch] );
...
//--------------------------------------------------------------------
void	Cls_GrRecRcvSoloV0::LcProcAdoFrame( void* A_PtrFrm, BOOL8 A_IsPreFlush )
{
	// local -------------------
		__u8		Tv_Ch;
		void*		Tv_PtrCol;
	...
	// code --------------------
		...
		if ( 0 != m_TimeSecLast )
		{
			Tv_SyncIdx	=	m_AdoSyncIdx - (E_GrRecRcvSoloV0SyncCnt >> 1);
			if ( 0 > Tv_SyncIdx )
			{
				Tv_SyncIdx	=	Tv_SyncIdx + E_GrRecRcvSoloV0SyncCnt;
			}
			...
			{
				//packet reserved
				m_AdoSyncTbl[Tv_SyncIdx].ChTbl[Tv_Ch].HndlNrm	=	E_GrDvrFsoHndlNone;
				m_AdoSyncTbl[Tv_SyncIdx].ChTbl[Tv_Ch].HndlMir	=	E_GrDvrFsoHndlNone;
				//check record able
				if ( 0 != (Tv_MaskCh & m_AdoRecMap) )
				{
					...
					if ( (0 != m_EvtTbl[Tv_Ch]) || A_IsPreFlush )
					{
						...
						m_FsMng->PacketReserve( sizeof(St_GrRecRcvSoloV0AdoPkt), 
																		&m_AdoSyncTbl[Tv_SyncIdx].ChTbl[Tv_Ch].HndlNrm, &m_AdoSyncTbl[Tv_SyncIdx].ChTbl[Tv_Ch].HndlMir );
					}
				}

				//write packet
				if ( (E_GrDvrFsoHndlNone != m_AdoSyncTbl[m_AdoSyncIdx].ChTbl[Tv_Ch].HndlNrm) || (E_GrDvrFsoHndlNone != m_AdoSyncTbl[m_AdoSyncIdx].ChTbl[Tv_Ch].HndlMir) )
				{
					//make packet
					m_AdoPktBuf.TailMark	=	E_GrDvrRecTailMarkId | sizeof(St_GrRecRcvSoloV0AdoPkt);	//tail
					//set header
					...
					//write normal
					if ( E_GrDvrFsoHndlNone != m_AdoSyncTbl[m_AdoSyncIdx].ChTbl[Tv_Ch].HndlNrm )
					{
			...
			//update info
			m_AdoSyncIdx ++;
			if ( E_GrRecRcvSoloV0SyncCnt <= m_AdoSyncIdx )
			{
				m_AdoSyncIdx	=	0;
			}
		}
...
//--------------------------------------------------------------------
void	Cls_GrRecRcvSoloV0::PreframeTimeSet( __u32 A_Sec )
{
		m_PrefrmTick	=	GrTimeMsToTick( (A_Sec + 1) * 1000 );
}
//--------------------------------------------------------------------
BOOL8	Cls_GrRecRcvSoloV0::LcProcNewestFrame( void )
{
	// local -------------------
		BOOL8	Tv_Result;
		__u32*	Tv_PtrFrm;
...
//--------------------------------------------------------------------
void	Cls_GrRecRcvSoloV0::LcFlushPreframes( void )
{
	// local -------------------
		__u32*	Tv_PtrFrm;
	// code --------------------
...
//--------------------------------------------------------------------
void	Cls_GrRecRcvSoloV0::LcProcLastFrame( void )
{
	// local -------------------
		BOOL8		Tv_IsRun;
		__u32*	Tv_PtrFrm;
...
//--------------------------------------------------------------------
void	Cls_GrRecRcvSoloV0::AdoRecMapSet( __u32 A_Map )
{
		m_AdoRecMap		=	A_Map;
}
//--------------------------------------------------------------------
void	Cls_GrRecRcvSoloV0::MotionPluginSet( __u8 A_Idx, Cls_GrTaskCli* A_ObjPlugin )
{
...
//--------------------------------------------------------------------
__u32	Cls_GrRecRcvSoloV0::VdoFrmAverageSizeGet( __u8 A_Ch )
{
		return	m_VdoFrmSizeTbl[A_Ch];
}
//--------------------------------------------------------------------
BOOL8	Cls_GrRecRcvSoloV0::VssAdd( __u8 A_Ch, __u8 A_Cmd, BOOL8 A_IsArmed, __u8 A_Etty, __u8 A_CstFrmCnt )
{
	// local -------------------
		BOOL8	Tv_Result;
		__u8	Tv_Idx;
		...
		__u8	Tv_FrmCnt;
		__u32	Tv_Mili;
		Ptr_GrRecRcvSoloV0VssRsv	Tv_PtrItm;
	// code --------------------
		Tv_Result	=	FALSE;
		...
		{
			//allocate queue
			Tv_PtrItm	=	(Ptr_GrRecRcvSoloV0VssRsv)GrBufQueGetInPtr( (Ptr_GrBufQue)m_VssQue[A_Ch] );
			if ( NULL != Tv_PtrItm )
			{
				...
				if ( E_GrRecRcvSoloV0VssHeadCnt > m_VssTbl[A_Ch].His[Tv_Idx].PfrmCnt )
				{
					Tv_Idx	=	Tv_Idx ^ 1;
				...
				if ( E_GrDvrFsoHndlNone != Tv_Hndl )
				{
					...
					Tv_PtrItm->FrmCnt		=	Tv_FrmCnt + E_GrRecRcvSoloV0VssTailCnt;
					if ( 0 != A_CstFrmCnt )
					{
				...
				{
					DbgMsgPrint( "Cls_GrRecRcvSoloV0::VssAdd - not enough pre-frame.\n" );
				}
			}
			else
			{
				DbgMsgPrint( "Cls_GrRecRcvSoloV0::VssAdd - not enough reserve queue.\n" );
			}
		}
		else
		{
			...
			m_VssTbl[A_Ch].His[Tv_Idx].Hndl			=	E_GrDvrFsoHndlNone;
			//check privous able
			Tv_Idx	=	Tv_Idx ^ 1;
			if ( (E_GrDvrFsoHndlNone != Tv_Hndl) && (E_GrDvrFsoHndlNone != m_VssTbl[A_Ch].His[Tv_Idx].Hndl) )
			{
				Tv_FrmCnt	=	Tv_FrmCnt + m_VssTbl[A_Ch].His[Tv_Idx].PfrmCnt;
				Tv_Hndl		=	m_VssTbl[A_Ch].His[Tv_Idx].Hndl;
				// reset
...
//--------------------------------------------------------------------
void	Cls_GrRecRcvSoloV0::LcMarkEvtPreframes( __u8 A_Ch, __u32 A_Evt )
{
	// local -------------------
		__u32*	Tv_PtrFrm;
		__u8		Tv_Ch;
