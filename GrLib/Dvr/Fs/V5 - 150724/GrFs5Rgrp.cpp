...
//--------------------------------------------------------------------
Cls_GrFs5Rgrp::Cls_GrFs5Rgrp(__u8 A_GroupType, __u8 A_DskMap, BOOL8 A_IsOvrtMode, BOOL8 A_IsCchRsvAble)
{
	// local -------------------
	// code --------------------
		//init
		...
		m_DiskId			=	V_GrFs5Env.DiskBaseId;
...
//--------------------------------------------------------------------
void	Cls_GrFs5Rgrp::LcCchMinInit(void)
{
	// local -------------------
		__u32		Tv_ItmIdx;
	// code --------------------
		...
		m_CchMin.Cnt		=	V_GrFs5Env.HwBufs[m_GrpType].CchMinCnt;
		if(0 == m_CchMin.Cnt)
		{
			DbgMsgPrint("Cls_GrFs5Rgrp::LcCchMinInit - min cache count is zero!\n");
			return;
		}
		...
		for(Tv_ItmIdx = 0; Tv_ItmIdx < m_CchMin.Cnt; Tv_ItmIdx++)
		{
			...
			m_CchMin.Items[Tv_ItmIdx].PtrBuf	=	(Ptr_GrFs5MinuteInfo)V_GrFs5Env.HwBufs[m_GrpType].CchMinBuf[Tv_ItmIdx];
		}

}
//--------------------------------------------------------------------
void	Cls_GrFs5Rgrp::LcCchMinFinish(void)
{
	// local -------------------
		__u32	Tv_ItmIdx;
	// code --------------------
...
//--------------------------------------------------------------------
void	Cls_GrFs5Rgrp::LcCchDataInit(void)
{
	// local -------------------
		__u32		Tv_ItmIdx;
	// code --------------------
		...
		// build data cache
		m_CchData.Cnt	=	V_GrFs5Env.HwBufs[m_GrpType].CchDataCnt;
		if(0 == m_CchData.Cnt)
		{
			DbgMsgPrint("Cls_GrFs5Rgrp::LcCchDataInit - data cache count is zero!\n");
			return;
		}
		...
		for(Tv_ItmIdx = 0; Tv_ItmIdx < m_CchData.Cnt; Tv_ItmIdx++)
		{
			...
			m_CchData.Items[Tv_ItmIdx].PtrBuf	=	V_GrFs5Env.HwBufs[m_GrpType].CchDataBuf[Tv_ItmIdx];
		}

}
//--------------------------------------------------------------------
void	Cls_GrFs5Rgrp::LcCchDataFinish(void)
{
	// local -------------------
		__u32		Tv_ItmIdx;
	// code --------------------
...
//--------------------------------------------------------------------
void	Cls_GrFs5Rgrp::LcGrpDskTblBuild(void)
{
	// local -------------------
		__u8	Tv_Mask;
		__u8	Tv_DskIdx;
...
//--------------------------------------------------------------------
__u32	Cls_GrFs5Rgrp::LcGrpCalcSeqId(__u8 A_DskIdx, __u32 A_BlkIdx, __u32 A_TimeSt)
{
	// local -------------------
		__u32	Tv_Result;
		__u32	Tv_DskIdx;
	...
	// code --------------------
		...
		{
			Tv_PtrBi	=	(Ptr_GrFs5BlkInfoBase)m_DiskTbl[Tv_DskIdx]->BlkInfoTableGet(&Tv_BlkCnt);
			if(NULL != Tv_PtrBi)
			{
				for(Tv_BlkIdx = 1; Tv_BlkIdx < Tv_BlkCnt; Tv_BlkIdx++)
				{
					if(E_GrFs5BlkStatBad != Tv_PtrBi[Tv_BlkIdx].Db.BlkStat)
					{
...
//--------------------------------------------------------------------
void	Cls_GrFs5Rgrp::LcGrpSeqIdRebuild(void)
{
	// local -------------------
		__u32	Tv_DskIdx;
		__u32	Tv_BlkCnt;
							...
							if(E_GrFs5BlkStatBad != Tv_PtrBi[Tv_BlkIdx].Db.BlkStat)
							{
								...
								if(0 != Tv_PtrBi[Tv_BlkIdx].Db.InSize)
								{
									...
									else
									{
										...
										DbgMsgPrint("Cls_GrFs5Rgrp::LcGrpSeqIdRebuild - disk %d block %d info bad\n",
...
//--------------------------------------------------------------------
Def_GrErrCode	Cls_GrFs5Rgrp::LogAdd(Ptr_GrDvrLogItm A_PtrItem)
{
	// local -------------------
	...
	// code --------------------
		...
		if((0 > m_RecBlkEnv.OfsLog) || (E_GrFs5FrmLogPoolItemCnt <= m_RecBlkEnv.Log.Head.InCnt))
		{
			// try create
...
//--------------------------------------------------------------------
BOOL8	Cls_GrFs5Rgrp::LcGrpBlkFindOldest(__s32* A_PtrRtDskIdx, __s32* A_PtrRtBlkIdx, __u32* A_PtrRtSeqId, __u8* A_PtrRtOvwtId)
{
	// local -------------------
		BOOL8	Tv_Result;
		__u8	Tv_DskIdx;
...
//--------------------------------------------------------------------
BOOL8	Cls_GrFs5Rgrp::LcGrpBlkFindNewest(__s32* A_PtrRtDskIdx, __s32* A_PtrRtBlkIdx, __u32* A_PtrRtSeqId)
{
	// local -------------------
		BOOL8	Tv_Result;
		__u8	Tv_DskIdx;
...
//--------------------------------------------------------------------
BOOL8	Cls_GrFs5Rgrp::LcGrpMibUpdate(void)
{
	// local -------------------
		BOOL8	Tv_Result;
		__u8	Tv_DskIdx;
	...
	// code --------------------
		...
		if(0 != m_DiskCnt)
		{
			for( Tv_DskIdx = 0; Tv_DskIdx < m_DiskCnt; Tv_DskIdx++)
			{
				...
				{
					// get MIB
					if(E_GrFs5Fcc == Tv_PtrBi[E_GrFs5BlkIdxMib].Mib.Fcc)
					{
						// update last record sequence
						Tv_PtrBi[E_GrFs5BlkIdxMib].Mib.SeqLast	=	m_RecSeqId;
						Tv_PtrBi[E_GrFs5BlkIdxMib].Mib.SeqOvwt	=	m_RecSeqOvwt;

						// write
						if(E_GrErrNone == m_DiskTbl[Tv_DskIdx]->BlkInfoWriteByIdx(E_GrFs5BlkIdxMib))
						{
							Tv_Result	=	TRUE;
						}
					}
...
//--------------------------------------------------------------------
__s32	Cls_GrFs5Rgrp::LcDskBlkFindEmpty(__u32 A_DskIdx)
{
	// local -------------------
		__s32	Tv_Result;
		__u32	Tv_BlkCnt;
	...
	// code --------------------
		...
		{
			Tv_PtrBi	=	(Ptr_GrFs5BlkInfoBase)m_DiskTbl[A_DskIdx]->BlkInfoTableGet(&Tv_BlkCnt);
			if(NULL != Tv_PtrBi)
			{
				for( Tv_BlkIdx = 1; Tv_BlkIdx < Tv_BlkCnt; Tv_BlkIdx++)
				{
					// check block status
					if(E_GrFs5BlkStatNormal == Tv_PtrBi[Tv_BlkIdx].Db.BlkStat)
					{
						if(0 == Tv_PtrBi[Tv_BlkIdx].Db.InSize)
						{
							// found
...
//--------------------------------------------------------------------
BOOL8	Cls_GrFs5Rgrp::LcGrpBlkFindEmpty(__s32* A_PtrRtDskIdx, __s32* A_PtrRtBlkIdx)
{
	// local -------------------
		BOOL8	Tv_Result;
		__u8	Tv_DskIdx;
...
//--------------------------------------------------------------------
Def_GrErrCode	Cls_GrFs5Rgrp::LcGrpRecBlkClose(BOOL8 A_IsFull, BOOL8 A_IsBad)
{
	// local -------------------
		BOOL8	Tv_IsDskStat;
	...
	// code --------------------
		...
		if((0 > m_RecDskIdx) || (0 > m_RecBlkIdx))
		{
			DbgMsgPrint("Cls_GrFs5Rgrp::LcGrpRecBlkClose - record not opened.\n");
		...
		if(!A_IsBad)
		{
			...
			if(A_IsFull)
			{
				m_RecBlkEnv.PtrDb->BlkStat		=	E_GrFs5BlkStatFull;
		...
		if(A_IsBad)
		{
			m_RecBlkEnv.PtrDb->BlkStat		=	E_GrFs5BlkStatBad;
			m_RecBlkEnv.IsDbChg	=	TRUE;
			Tv_IsDskStat	=	TRUE;
		}
		...
		if(m_RecBlkEnv.IsDbChg)
		{
			...
			{
				DbgMsgPrint("Cls_GrFs5Rgrp::LcGrpRecBlkClose - disk %d block %d block info write fail!\n",
...
//--------------------------------------------------------------------
Def_GrErrCode	Cls_GrFs5Rgrp::LcGrpRecBlkOpen(__u32 A_DskIdx, __u32 A_BlkIdx)
{
	// local -------------------
		__u32	Tv_BlkCnt;
		Ptr_GrFs5BlkInfoBase	Tv_PtrBi;
	// code --------------------
		...
		if((0 <= m_RecDskIdx) || (0 <= m_RecBlkIdx))
		{
			DbgMsgPrint("Cls_GrFs5Rgrp::LcGrpRecBlkCreate - record already open!\n");
			return	E_GrErrBadParam;
		}
		...
		{
			DbgMsgPrint("Cls_GrFs5Rgrp::LcGrpRecBlkCreate - Bad disk index %d value!\n", A_DskIdx);
			return	E_GrErrBadParam;
		}
		...
		{
			DbgMsgPrint("Cls_GrFs5Rgrp::LcGrpRecBlkCreate - Disk %d can not open!\n", m_DiskTbl[A_DskIdx]->DevIdxGet());
		...
		{
			DbgMsgPrint("Cls_GrFs5Rgrp::LcGrpRecBlkCreate - Bad block index %d (Disk: %d)!\n",
				A_BlkIdx, m_DiskTbl[A_DskIdx]->DevIdxGet());
			return	E_GrErrBadParam;
		}

		// check block status
		if(E_GrFs5BlkStatBad == Tv_PtrBi[A_BlkIdx].Db.BlkStat)
		{
			DbgMsgPrint("Cls_GrFs5Rgrp::LcGrpRecBlkCreate - block (disk:%d,index:%d) status is bad!\n",
		...
		DbgMsgPrint("Cls_GrFs5Rgrp::LcGrpRecBlkOpen - disk:%d blk:%d seq:%d open!\n", m_RecDskIdx,m_RecBlkIdx,m_RecSeqId);

		return	E_GrErrNone;
}
//--------------------------------------------------------------------
Def_GrErrCode	Cls_GrFs5Rgrp::LcGrpRecBlkNew(__u32 A_DskIdx, __u32 A_BlkIdx)
{
	// local -------------------
		Def_GrErrCode	Tv_ErrCode;
		__u32	Tv_BlkCnt;
	...
	// code --------------------
		...
		if((0 <= m_RecDskIdx) || (0 <= m_RecBlkIdx))
		{
			DbgMsgPrint("Cls_GrFs5Rgrp::LcGrpRecBlkNew - record already open!\n");
			return	E_GrErrBadParam;
		}
		...
		if((__u32)m_DiskCnt <= A_DskIdx)
		{
			DbgMsgPrint("Cls_GrFs5Rgrp::LcGrpRecBlkNew - Bad disk index %d value!\n", A_DskIdx);
			return	E_GrErrBadParam;
		}
		...
		if((NULL == Tv_PtrBi) || (0 == Tv_BlkCnt))
		{
			DbgMsgPrint("Cls_GrFs5Rgrp::LcGrpRecBlkNew - Disk %d can not open!\n", m_DiskTbl[A_DskIdx]->DevIdxGet());
			return	E_GrErrDiskStatBad;
		}
		...
		if(Tv_BlkCnt <= A_BlkIdx)
		{
			DbgMsgPrint("Cls_GrFs5Rgrp::LcGrpRecBlkNew - Bad block index %d (Disk: %d)!\n",
				A_BlkIdx, m_DiskTbl[A_DskIdx]->DevIdxGet());
			return	E_GrErrBadParam;
		}
		...
		if(E_GrFs5BlkStatBad == Tv_PtrBi[A_BlkIdx].Db.BlkStat)
		{
			DbgMsgPrint("Cls_GrFs5Rgrp::LcGrpRecBlkNew - block (disk:%d,index:%d) status is bad!\n",
				m_DiskTbl[A_DskIdx]->DevIdxGet(), A_BlkIdx);
			return	E_GrErrDiskBadSector;
		}
		...
		// update overwrite id
		if((E_GrFs5BlkStatNormal != Tv_PtrBi[A_BlkIdx].Db.BlkStat) || (0 != Tv_PtrBi[A_BlkIdx].Db.InSize))
		{
			Tv_PtrBi[A_BlkIdx].Db.OvwtId ++;
		}

		// build new sequence id
		Tv_PtrBi[A_BlkIdx].Db.BlkStat					=	E_GrFs5BlkStatNormal;
		...
		// success
#if 0
#ifdef E_GrFs5RgrpDbgMsgView
		DbgMsgPrint("Cls_GrFs5Rgrp::LcGrpRecBlkNew - block (disk:%d,index:%d,seq id:%d) new allocated.\n",
...
//--------------------------------------------------------------------
void	Cls_GrFs5Rgrp::LcGrpRecLogPoolPktBuild(__s32 A_OfsPrv)
{
	// local -------------------
	// code --------------------
		m_RecBlkEnv.Log.Head.Base.Fcc				=	E_GrDvrFrmHeadFcc;
		m_RecBlkEnv.Log.Head.Base.Ch				=	0xFF;
		m_RecBlkEnv.Log.Head.Base.DataSize	=	sizeof(St_GrDvrLogItm)* E_GrFs5FrmLogPoolItemCnt;
		...
		GrDumyZeroMem(m_RecBlkEnv.Log.LogTbl, sizeof(St_GrDvrLogItm)* E_GrFs5FrmLogPoolItemCnt);

		m_RecBlkEnv.Log.Tail.Fcc						=	E_GrDvrFrmTailFcc;
		m_RecBlkEnv.Log.Tail.PktSize				=	sizeof(St_GrFs5FrmLogPool);

}
//--------------------------------------------------------------------
Def_GrErrCode	Cls_GrFs5Rgrp::LcGrpRecLogPoolOpen(__u32 A_DskIdx, __u32 A_BlkIdx)
{
	// local -------------------
		Def_GrErrCode	Tv_ErrCode;
		Ptr_GrFs5DbBlkInfo	Tv_PtrBi;
	// code --------------------
		// get block info
		Tv_PtrBi	=	LcDskDbBlkInfoGet(A_DskIdx, A_BlkIdx);
		if(NULL == Tv_PtrBi)
		{
			DbgMsgPrint("Cls_GrFs5Rgrp::LcGrpRecLogPoolOpen - can not get block info!\n");
			return	E_GrErrBadParam;
		}
		...
		{
			DbgMsgPrint("Cls_GrFs5Rgrp::LcGrpRecLogPoolOpen - last logblock not found, create new.\n");
			LcGrpRecLogPoolPktBuild(-1);
			return	E_GrErrNone;
		}
		...
		Tv_ErrCode	=	LcDskReadToBuf(A_DskIdx, A_BlkIdx, (__u32)Tv_PtrBi->OfsLastLog,
			sizeof(St_GrFs5FrmLogPool), &m_RecBlkEnv.Log);
		if(E_GrErrNone == Tv_ErrCode)
		{
			// check log packet
			if((E_GrDvrFrmHeadFcc != m_RecBlkEnv.Log.Head.Base.Fcc) || (E_GrDvrFrmTypeLogPool != m_RecBlkEnv.Log.Head.Base.Type))
			{
				DbgMsgPrint("Cls_GrFs5Rgrp::LcGrpRecLogPoolOpen - bad log frame found!\n");
				// delete log
...
//--------------------------------------------------------------------
Def_GrErrCode	Cls_GrFs5Rgrp::LcGrpRecLogPoolWrite(void)
{
	// local -------------------
		Def_GrErrCode	Tv_ErrCode;
	// code --------------------
		...
		{
			DbgMsgPrint("Cls_GrFs5Rgrp::LcGrpRecLogPoolWrite - log pool not registed!\n");
			return	E_GrErrFail;
		}
...
//--------------------------------------------------------------------
Def_GrErrCode	Cls_GrFs5Rgrp::LcGrpRecLogPoolAdd(void)
{
	// local -------------------
		__s32	Tv_PrvOfs;
	// code --------------------
		...
		{
			DbgMsgPrint("Cls_GrFs5Rgrp::LcGrpRecLogPoolAdd - block full!\n");
			m_RecBlkEnv.OfsLog	=	-1;
			return	E_GrErrDiskFull;
		}

		// check allocate able
		if(((__u32)E_GrFs5BlkSize - m_RecBlkEnv.PtrDb->InSize) < sizeof(St_GrFs5FrmLogPool))
		{
			DbgMsgPrint("Cls_GrFs5Rgrp::LcGrpRecLogPoolAdd - block full!\n");
			m_RecBlkEnv.OfsLog	=	-1;
			return	E_GrErrDiskFull;
		...
		m_RecBlkEnv.PtrDb->InSize	=	m_RecBlkEnv.PtrDb->InSize + sizeof(St_GrFs5FrmLogPool);
		if((__u32)E_GrFs5BlkSize <= m_RecBlkEnv.PtrDb->InSize)
		{
		...
		if(E_GrErrNone != LcGrpRecLogPoolWrite())
		{
			...
			m_RecBlkEnv.OfsLog	=	-1;
			DbgMsgPrint("Cls_GrFs5Rgrp::LcGrpRecLogPoolAdd - data write fail!\n");
		}
...
//--------------------------------------------------------------------
Def_GrErrCode	Cls_GrFs5Rgrp::LcGrpRecAddTimeByRbw(__u32 A_Time, __u32 A_Ofs, Ptr_GrFs5RgrpRbwChCtrl A_PtrRbwCi)
{
	// local -------------------
...
//--------------------------------------------------------------------
__u8	Cls_GrFs5Rgrp::StatusGet(BOOL8 A_IsRefresh)
{
		if(A_IsRefresh)
		{
			LcGrpStatUpdate();
...
//--------------------------------------------------------------------
Def_GrErrCode	Cls_GrFs5Rgrp::LcGrpRecAddBlkTimeRange(__u32 A_TimeMin, __u32 A_TimeMax)
{
	// local -------------------
		__u8	Tv_Result;
	// code --------------------
...
//--------------------------------------------------------------------
__u32	Cls_GrFs5Rgrp::LcGrpRecFreeSizeGet(void)
{
	// local -------------------
		__u32	Tv_Result;
	// code --------------------
		...
		{
			// check full
			if (E_GrFs5BlkStatNormal == m_RecBlkEnv.PtrDb->BlkStat)
			{
				Tv_Result				=	(__u32)E_GrFs5BlkSize - m_RecBlkEnv.PtrDb->InSize;
			}
		}
...
//--------------------------------------------------------------------
BOOL8	Cls_GrFs5Rgrp::LcGrpBlkFindBySeqFw(__u32 A_SeqId, __s32* A_PtrRtDskIdx, __s32* A_PtrRtBlkIdx, __u32* A_PtrRtBlkSeq, __u8* A_PtrRtOvwtId)
{
	// local -------------------
		BOOL8	Tv_Result;
		BOOL8	Tv_IsExit;
...
//--------------------------------------------------------------------
Def_GrErrCode	Cls_GrFs5Rgrp::LcGrpReadSctlFwByHndl(Ptr_GrFs5SeekCtrl A_PtrCtrl, __u64 A_Hndl)
{
	// local -------------------
	...
	// code --------------------
		...
		// decode handle
		GrFs5HndlDec(A_Hndl, &Tv_DskIdx, (__u32*)&Tv_BlkOfs, &Tv_BlkIdx, &Tv_OvwtId);

		// patch offset
		Tv_CchOfs	=	Tv_BlkOfs & E_GrFs5RgrpCchDataOfsMask;
		Tv_BlkOfs	=	Tv_BlkOfs & E_GrFs5RgrpCchDataAdrMask;

		// read 
		Tv_Hndl		=	GrFs5HndlEnc(Tv_DskIdx, Tv_BlkOfs, Tv_BlkIdx, Tv_OvwtId);
		// get cache
		...
		// go next address
		Tv_BlkOfs	=	Tv_BlkOfs + E_GrFs5RgrpCchDataBufSize;
		...
		while(TRUE)
		{
			...
			// check exist block
			if(E_GrFs5BlkOfsLimit > Tv_BlkOfs)
			{
				Tv_Hndl	=	GrFs5HndlEnc(Tv_DskIdx, Tv_BlkOfs, Tv_BlkIdx, Tv_OvwtId);
				// read cache block
				Tv_ErrCode	=	LcCchDataGetByHndlToSctl(A_PtrCtrl, Tv_Hndl);
			...
			if(!LcGrpBlkFindBySeqFw(Tv_BlkSeq, &Tv_DskIdx, &Tv_BlkIdx, &Tv_BlkSeq, &Tv_OvwtId))
			{
				// finished
#ifdef E_GrFs5RgrpDbgMsgView
				DbgMsgPrint("Cls_GrFs5Rgrp::LcGrpReadSctlFwByHndl - next sequence id (%d) not found\n", Tv_BlkSeq);
#endif
				return	E_GrErrEndOfData;
			}
...
//--------------------------------------------------------------------
Def_GrErrCode	Cls_GrFs5Rgrp::LcGrpReadSctlBwByHndl(Ptr_GrFs5SeekCtrl A_PtrCtrl, __u64 A_Hndl)
{
	// local -------------------
		Def_GrCalPtr	Tv_ErrCode;
		__u8	Tv_OvwtId;
	...
	// code --------------------
		...
		while(TRUE)
		{
			...
			if(!LcGrpBlkFindBySeqBw(Tv_BlkSeq, &Tv_DskIdx, &Tv_BlkIdx, &Tv_BlkSeq, &Tv_OvwtId, (__u32*)&Tv_BlkOfs))
			{
				// finished
#ifdef E_GrFs5RgrpDbgMsgView
				DbgMsgPrint("Cls_GrFs5Rgrp::LcGrpReadSctlBwByHndl - next sequence id (%d) not found\n", Tv_BlkSeq);
#endif
				return	E_GrErrEndOfData;
			}
...
//--------------------------------------------------------------------
Def_GrErrCode	Cls_GrFs5Rgrp::ReadFwByTime(Ptr_GrFs5SeekCtrl A_PtrCtrl, Def_GrTime A_Time)
{
	// local -------------------
		__u8	Tv_OvwtId;
		__s32	Tv_DskIdx;
	...
	// code --------------------
		...
		if(0 == A_Time)
		{
			DbgMsgPrint("Cls_GrFs5Rgrp::ReadFwByTime - input zero time!\n");
			return	E_GrErrBadParam;
		}
...
//--------------------------------------------------------------------
Def_GrErrCode	Cls_GrFs5Rgrp::ReadFwStart(Ptr_GrFs5SeekCtrl A_PtrCtrl)
{
	// local -------------------
		__u8	Tv_OvwtId;
		__s32	Tv_DskIdx;
	...
	// code --------------------
		...
		// read 
		Tv_Hndl		=	GrFs5HndlEnc(Tv_DskIdx, 0, Tv_BlkIdx, Tv_OvwtId);

		return	LcGrpReadSctlFwByHndl(A_PtrCtrl, Tv_Hndl);

}
//--------------------------------------------------------------------
void	Cls_GrFs5Rgrp::LcCchDataUnlockByIdx(__u32 A_CchIdx)
{
	// local -------------------
	// code --------------------
		...
		{
			DbgMsgPrint("Cls_GrFs5Rgrp::LcCchDataUnlockByIdx - cache already unlocked!\n");
			return;
		}
...
//--------------------------------------------------------------------
void	Cls_GrFs5Rgrp::LcCchDataUnlockByHndl(__u64 A_Hndl)
{
	// local -------------------
		__s32	Tv_CchIdx;
	// code --------------------
		...
		if(0 > Tv_CchIdx)
		{
			DbgMsgPrint("Cls_GrFs5Rgrp::LcCchDataUnlockByHndl - handle not found!\n");
			return;
		}
...
//--------------------------------------------------------------------
void	Cls_GrFs5Rgrp::LcCchDataUnlockBySctl(Ptr_GrFs5SeekCtrl A_PtrCtrl)
{
	// local -------------------
	// code --------------------
		// check valid cache
...
//--------------------------------------------------------------------
void	Cls_GrFs5Rgrp::LcCchDataMarkAccByIdx(__u32 A_CchIdx, BOOL8 A_IsWrite)
{
	// local -------------------
	// code --------------------
		if(m_CchData.Cnt <= A_CchIdx)
		...
		m_CchData.Items[A_CchIdx].AccTime	=	V_GrFs5Env.Tick100ms;
...
//--------------------------------------------------------------------
void	Cls_GrFs5Rgrp::LcCchDataMarkAccByHndl(__u64 A_Hndl, BOOL8 A_IsWrite)
{
	// local -------------------
		__s32	Tv_CchIdx;
	// code --------------------
		if(E_GrDvrRecHndlNone == A_Hndl)
		{
			DbgMsgPrint("Cls_GrFs5Rgrp::LcCchDataMarkAccByHndl - none handle input!\n");
			return;
		}
		...
		if(0 > Tv_CchIdx)
		{
			DbgMsgPrint("Cls_GrFs5Rgrp::LcCchDataMarkAccByHndl - bad handle input!\n");
			return;
		}
...
//--------------------------------------------------------------------
Def_GrErrCode	Cls_GrFs5Rgrp::ReadFwByHndl(Ptr_GrFs5SeekCtrl A_PtrCtrl, __u64 A_Hndl)
{
	// local -------------------
		__u8	Tv_OvwtId;
		__s32	Tv_DskIdx;
	...
	// code --------------------
		...
		// make cache handle
		GrFs5HndlDec(A_Hndl, &Tv_DskIdx, &Tv_BlkOfs, &Tv_BlkIdx, &Tv_OvwtId);
		Tv_CchOfs	=	Tv_BlkOfs & E_GrFs5RgrpCchDataOfsMask;
		Tv_BlkOfs	=	Tv_BlkOfs & E_GrFs5RgrpCchDataAdrMask;
...
//--------------------------------------------------------------------
void	Cls_GrFs5Rgrp::LcCchDataReserve(BOOL8 A_IsDirBack, __u8 A_ReqCnt, __u64 A_HndlBase)
{
	// local -------------------
		__u8	Tv_ReqCnt;
		__u8	Tv_ReqIdx;
	...
	// code --------------------
		...
		// patch request count
		Tv_ReqCnt	=	A_ReqCnt;
		if(E_GrFs5RgrpCchDataMaxRsvd <= Tv_ReqCnt)
		{
			Tv_ReqCnt	=	E_GrFs5RgrpCchDataMaxRsvd;
		}

		// get base block info
		GrFs5HndlDec(A_HndlBase, &Tv_DskIdx, &Tv_BlkOfs, &Tv_BlkIdx, &Tv_OvwtId);
		...
		for( Tv_ReqIdx = 0; Tv_ReqIdx < Tv_ReqCnt; Tv_ReqIdx++)
		{
			if(Tv_IsAble)
			{
				...
				{
					// back direction
					if(E_GrFs5RgrpCchDataBufSize < Tv_BlkOfs)
					{
						Tv_BlkOfs	=	Tv_BlkOfs - E_GrFs5RgrpCchDataBufSize;
					}
					else
				...
				{
					// forward direction
					if((__u32)E_GrFs5BlkSize > Tv_BlkOfs)
					{
						Tv_BlkOfs	=	Tv_BlkOfs + E_GrFs5RgrpCchDataBufSize;
					}
					else
			...
			if(Tv_IsAble)
			{
				m_CchData.Rsvd.Items[Tv_ReqIdx].Hndl	=	GrFs5HndlEnc(Tv_DskIdx, Tv_BlkOfs, Tv_BlkIdx, Tv_OvwtId);
			}
			else
			{
...
//--------------------------------------------------------------------
Def_GrErrCode	Cls_GrFs5Rgrp::ReadFwNext(Ptr_GrFs5SeekCtrl A_PtrCtrl)
{
	// local -------------------
		Def_GrErrCode	Tv_ErrCode;
		__u8	Tv_OvwtId;
	...
	// code --------------------
		...
		{
#ifdef E_GrFs5RgrpDbgMsgView
			DbgMsgPrint("Cls_GrFs5Rgrp::ReadFwNext - bad handle input!\n");
#endif
			return	E_GrErrBadParam;
		}

		// get block info
		GrFs5HndlDec(A_PtrCtrl->Hndl, &Tv_DskIdx, &Tv_BlkOfs, &Tv_BlkIdx, &Tv_OvwtId);
		Tv_BlkSeq		=	A_PtrCtrl->BlkSeq;

		// check block offset
#ifdef E_GrFs5RgrpDbgMsgView
		if(0 != (E_GrFs5RgrpCchDataOfsMask & Tv_BlkOfs))
		{
			DbgMsgPrint("Cls_GrFs5Rgrp::ReadFwNext - handle address not aligned!\n");
			Tv_BlkOfs	=	Tv_BlkOfs & E_GrFs5RgrpCchDataAdrMask;
		}
#endif

		// check changed now block
		if(E_GrFs5RgrpCchDataBufSize > A_PtrCtrl->InSize)
		{
		...
		while(TRUE)
		{
			...
			if(!LcGrpBlkFindBySeqFw(Tv_BlkSeq, &Tv_DskIdx, &Tv_BlkIdx, &Tv_BlkSeq, &Tv_OvwtId))
			{
				// finished
#ifdef E_GrFs5RgrpDbgMsgView
				DbgMsgPrint("Cls_GrFs5Rgrp::ReadFwNext - next sequence id (%d) not found\n", Tv_BlkSeq);
#endif
				return	E_GrErrEndOfData;
			}
...
//--------------------------------------------------------------------
BOOL8	Cls_GrFs5Rgrp::LcGrpBlkFindBySeqBw(__u32 A_SeqId, __s32* A_PtrRtDskIdx, __s32* A_PtrRtBlkIdx, __u32* A_PtrRtBlkSeq, __u8* A_PtrRtOvwtId, __u32* A_PtrRtOfs)
{
	// local -------------------
		BOOL8	Tv_Result;
		BOOL8	Tv_IsExit;
		...
		if(0 != m_DiskCnt)
		{
			...
			for(Tv_DskIdx = 0; Tv_DskIdx < m_DiskCnt; Tv_DskIdx++)
			{
				...
				if(NULL != Tv_PtrBi)
				{
					...
					{
						// check block type
						if((E_GrFs5BlkStatBad != Tv_PtrBi[Tv_BlkIdx].Db.BlkStat) && (0 != Tv_PtrBi[Tv_BlkIdx].Db.SeqId))
						{
							// check sequence id
							if(Tv_PtrBi[Tv_BlkIdx].Db.SeqId == A_SeqId)
							{
...
//--------------------------------------------------------------------
Def_GrErrCode	Cls_GrFs5Rgrp::ReadBwByTime(Ptr_GrFs5SeekCtrl A_PtrCtrl, Def_GrTime A_Time)
{
	// local -------------------
		__u8	Tv_OvwtId;
		__s32	Tv_DskIdx;
	...
	// code --------------------
		...
		if(0 == A_Time)
		{
			DbgMsgPrint("Cls_GrFs5Rgrp::ReadBwByTime - input zero time!\n");
			return	E_GrErrBadParam;
		}
...
//--------------------------------------------------------------------
Def_GrErrCode	Cls_GrFs5Rgrp::ReadBwNext(Ptr_GrFs5SeekCtrl A_PtrCtrl)
{
	// local -------------------
		Def_GrErrCode	Tv_ErrCode;
		__u8	Tv_OvwtId;
	...
	// code --------------------
		...
		{
#ifdef E_GrFs5RgrpDbgMsgView
			DbgMsgPrint("Cls_GrFs5Rgrp::ReadBwNext - bad handle input!\n");
#endif
			return	E_GrErrBadParam;
		}

		// get block info
		GrFs5HndlDec(A_PtrCtrl->Hndl, &Tv_DskIdx, (__u32*)&Tv_BlkOfs, &Tv_BlkIdx, &Tv_OvwtId);
		Tv_BlkSeq		=	A_PtrCtrl->BlkSeq;
		...
		if(0 != (E_GrFs5RgrpCchDataOfsMask & Tv_BlkOfs))
		{
			DbgMsgPrint("Cls_GrFs5Rgrp::ReadBwNext - handle address not aligned!\n");
			Tv_BlkOfs	=	Tv_BlkOfs & E_GrFs5RgrpCchDataAdrMask;
		}
#endif
...
//--------------------------------------------------------------------
__u8	Cls_GrFs5Rgrp::GroupInfoGet(__u64* A_PtrRtTotal, __u64* A_PtrRtRec, BOOL8 A_IsRefresh)
{
	// local -------------------
	// code --------------------
		if(A_IsRefresh)
...
//--------------------------------------------------------------------
void	Cls_GrFs5Rgrp::RecTimeRangeGet(__u32* A_PtrRtTimeSt, __u32* A_PtrRtTimeEd)
{
	// local -------------------
	// code --------------------
...
//--------------------------------------------------------------------
__u32	Cls_GrFs5Rgrp::RecTimeLastGet(void)
{
		return	m_RecTimeLast;
}
//--------------------------------------------------------------------
__u32	Cls_GrFs5Rgrp::RecMonthDayMapGet(__u32 A_TimeMonth)
{
	// local -------------------
		__u32	Tv_Result;
		__u32	Tv_DskIdx;
		...
		Ptr_GrFs5MinuteInfo	Tv_MnutTbl;
		Def_GrErrCode	Tv_ErrCode;
	// code --------------------
		...
		for( Tv_DskIdx = 0; Tv_DskIdx < m_DiskCnt; Tv_DskIdx++)
		{
			...
			for(Tv_DayIdx = 0; Tv_DayIdx < E_GrFs5DayTblCntOfDisk; Tv_DayIdx++)
			{
				// day
...
//--------------------------------------------------------------------
BOOL8	Cls_GrFs5Rgrp::LcDskDayMinMapGet(__u32 A_DskIdx, __u32 A_TimeDay, Ptr_GrDvrRecMinMap A_PtrMinMap)
{
	// local -------------------
		BOOL8	Tv_Result;
		__u32	Tv_ChIdx;
...
//--------------------------------------------------------------------
__u64	Cls_GrFs5Rgrp::LcDskExpSizeGet(__u32 A_DskIdx, __u32 A_TimeSt, __u32 A_TimeEd, __u32 A_ChMap)
{
	// local -------------------
		__u64	Tv_Result;
		__u32	Tv_ChIdx;
...
//--------------------------------------------------------------------
Ptr_GrFs5DbBlkInfo	Cls_GrFs5Rgrp::LcDskDbBlkInfoGet(__u32 A_DskIdx, __u32 A_BlkIdx)
{
	// local -------------------
		Ptr_GrFs5DbBlkInfo		Tv_Result;
		Ptr_GrFs5BlkInfoBase	Tv_BiTbl;
		__u32	Tv_BlkCnt;
	// code --------------------
		Tv_Result	=	NULL;
...
//--------------------------------------------------------------------
BOOL8	Cls_GrFs5Rgrp::RecMinRatbTblGet(__u32 A_TimeDay, __u32 A_MinCnt, Ptr_GrDvrRecMinMap A_PtrMinMap)
{
	// local -------------------
		BOOL8	Tv_Result;
		__u32	Tv_DskIdx;
...
//--------------------------------------------------------------------
void	Cls_GrFs5Rgrp::RecFlush(void)
{
	// local -------------------
	// code --------------------
		...
		if(NULL != m_RecBlkEnv.PtrDb)
		{
			...
			if(m_RecBlkEnv.IsDbChg)
			{
				if((0 <= m_RecDskIdx) && (0 <= m_RecBlkIdx))
				{
					...
					if(E_GrErrNone != m_DiskTbl[m_RecDskIdx]->BlkInfoWriteByIdx((__u32)m_RecBlkIdx))
					{
						DbgMsgPrint("Cls_GrFs5Rgrp::RecFlush - disk %d block %d block info write fail!\n",
							m_DiskTbl[m_RecDskIdx]->DevIdxGet(), m_RecBlkIdx);
					}
				}
				m_RecBlkEnv.IsDbChg	=	FALSE;
...
//--------------------------------------------------------------------
void	Cls_GrFs5Rgrp::RtlLogReadProc(Ptr_GrFs5IoLogGetRtl A_PtrCtl, Ptr_GrFs5FrmLogPool A_PtrFrmBuf)
{
	// local -------------------
		Ptr_GrFs5BlkInfoBase Tv_PtrBi;
		__u32	Tv_BlkCnt;
	...
	// code --------------------
		...
		while(TRUE)
		{
			...
			// load frame
			Tv_ErrCode	=	LcDskReadToBuf(A_PtrCtl->DskIdx, A_PtrCtl->BlkIdx, (__u32)A_PtrCtl->LogOfs, sizeof(St_GrFs5FrmLogPool), A_PtrFrmBuf);
			if(E_GrErrNone != Tv_ErrCode)
			{
				// go next block
			...
			{
				// go next block
				DbgMsgPrint("Cls_GrFs5Rgrp::LogReadByCtrl - bad frame(d:%d b:%d) head, go next block!\n",
			...
			{
				DbgMsgPrint("Cls_GrFs5Rgrp::LogReadByCtrl - bad previous address (now:%08X , prv:%08X)!\n", A_PtrCtl->LogOfs, A_PtrFrmBuf->Head.OfsPrv);
				// go next block
				A_PtrCtl->LogOfs	=	-1;
				A_PtrCtl->BlkIdx ++;
...
//--------------------------------------------------------------------
__u64	Cls_GrFs5Rgrp::RecExpectSizeGet(__u32 A_TimeSt, __u32 A_TimeEd, __u32 A_ChMap)
{
	// local -------------------
		__u64	Tv_Result;
...
//--------------------------------------------------------------------
void	Cls_GrFs5Rgrp::RecBufAdd(iovec* A_PtrIovs, __u32 A_Cnt)
{
	// local -------------------
		__u8	Tv_ErrCode;
		BOOL8	Tv_IsFull;
...
//--------------------------------------------------------------------
Def_GrErrCode	Cls_GrFs5Rgrp::LcGrpRbwWriteToRecBlk(void)
{
	// local -------------------
		Def_GrErrCode	Tv_Result;
		__u8	Tv_Ch;
	...
	// code --------------------
		...
		// find current cache
		Tv_BlkOfs	=	m_RecBlkEnv.PtrDb->InSize & E_GrFs5RgrpCchDataAdrMask;
		Tv_CchOfs	=	m_RecBlkEnv.PtrDb->InSize & E_GrFs5RgrpCchDataOfsMask;
		Tv_Hndl		=	GrFs5HndlEnc(m_RecDskIdx, Tv_BlkOfs, m_RecBlkIdx, m_RecBlkEnv.PtrDb->OvwtId);

		Tv_CchIdx	=	LcCchDataGetByHndl(Tv_Hndl, FALSE, &Tv_CchIns, &Tv_Result);
		if(0 > Tv_CchIdx)
		{
			GrStrTimeToStr(Tv_TestStr, GrTimeGet(&Tv_Mili, &Tv_IsSm));
			DbgMsgPrint("Cls_GrFs5Rgrp::LcGrpRbwWriteToRecBlk - TEST 2 : invalid cchidx time = %s\n", Tv_TestStr);
			return	Tv_Result;
		}
		...
		{
			GrStrTimeToStr(Tv_TestStr, GrTimeGet(&Tv_Mili, &Tv_IsSm));
			DbgMsgPrint("Cls_GrFs5Rgrp::LcGrpRbwWriteToRecBlk - cin (%08X) and cof (%08X) mismatch!\n", Tv_CchIns, Tv_CchOfs);
			DbgMsgPrint("Cls_GrFs5Rgrp::LcGrpRbwWriteToRecBlk - Time = %s\n", Tv_TestStr);
		}
		...
		while (m_RbwCtl.IocPrgs < m_RbwCtl.IocCnt)
		{
			...
			// check add able
			if((__u32)E_GrFs5BlkSize < (m_RecBlkEnv.PtrDb->InSize + (__u32)m_RbwCtl.IocTbl[m_RbwCtl.IocPrgs].iov_len))
			{
				// finish
			...
			// do copy
			if(E_GrFs5RgrpCchDataBufSize >= (Tv_CchOfs + (__u32)m_RbwCtl.IocTbl[m_RbwCtl.IocPrgs].iov_len))
			{
				// direct copy
			...
			else
			{
				...
				// divide
				Tv_CpHead		=	E_GrFs5RgrpCchDataBufSize - Tv_CchOfs;
				...
				{
					GrStrTimeToStr(Tv_TestStr, GrTimeGet(&Tv_Mili, &Tv_IsSm));
					DbgMsgPrint("Cls_GrFs5Rgrp::LcGrpRbwWriteToRecBlk - TEST 3 : invalid cchidx. time = %s\n", Tv_TestStr);
					return	Tv_Result;
				}
				...
				{
					GrStrTimeToStr(Tv_TestStr, GrTimeGet(&Tv_Mili, &Tv_IsSm));
					DbgMsgPrint("Cls_GrFs5Rgrp::LcGrpRbwWriteToRecBlk - cin (%08X) is not zero! time = %s\n", Tv_CchIns, Tv_TestStr);
			...
			if ((E_GrDvrFrmTypeVdoP4x >= Tv_PtrFrmUnk->Type) || (E_GrDvrFrmTypeAdo == Tv_PtrFrmUnk->Type))
			{
				...
				{
					// build new time base
					Tv_TmCanMin		=	Tv_PtrFrmUnk->TimeSec - (Tv_PtrFrmUnk->TimeSec % E_GrFs5TimeMapSecUnit);
				...
				{
					// check range
					if ((Tv_TmCanMin > Tv_PtrFrmUnk->TimeSec) || ((Tv_TmCanMin + E_GrFs5TimeMapSecUnit) <= Tv_PtrFrmUnk->TimeSec))
					{
						// out of range
						break;
					}
				...
				// add backup size
				m_RbwCtl.ChTbl[Tv_Ch].BkSize	=	m_RbwCtl.ChTbl[Tv_Ch].BkSize + Tv_PtrFrmUnk->DataSize + V_GrFs5Env.BkupHdSize;
			}
...
//--------------------------------------------------------------------
__s32	Cls_GrFs5Rgrp::LcCchMinFindInCache(__u32 A_DskIdx, __u32 A_DayIdx)
{
	// local -------------------
		__s32	Tv_Result;
		__u32	Tv_ItmIdx;
...
//--------------------------------------------------------------------
__s32	Cls_GrFs5Rgrp::LcCchMinLoadToCache(__u32 A_DskIdx, __u32 A_DayIdx, Def_GrErrCode* A_PtrRtErrCode, BOOL8 A_IsReadSkip)
{
	// local -------------------
		__s32	Tv_Result;
		__u32	Tv_ItmIdx;
	...
	// code --------------------
		...
		{
			// do load
			Tv_BlkOfs	=	E_GrFs5BlkOfsMinuteTbl + (A_DayIdx * E_GrFs5MitDaySize);
			*A_PtrRtErrCode	=	m_DiskTbl[A_DskIdx]->BlkReadPage(E_GrFs5BlkIdxMib, Tv_BlkOfs, m_CchMin.Items[Tv_Result].PtrBuf,
				E_GrFs5MitDaySize);
		...
		{
			// patch cache
			m_CchMin.Items[Tv_Result].AccTime	=	V_GrFs5Env.Tick100ms;		// update tick
...
//--------------------------------------------------------------------
Ptr_GrFs5MinuteInfo	Cls_GrFs5Rgrp::LcDskMinuteTblGetByIdx(__u32 A_DskIdx, __u32 A_DayIdx, Def_GrErrCode* A_PtrRtErrCode, BOOL8 A_IsReadSkip)
{
	// local -------------------
		__s32	Tv_CchIdx;
	...
	// code --------------------
		// check disk index
		if((E_GrFs5DayTblCntOfDisk <= A_DayIdx) || ((__u32)m_DiskCnt <= A_DskIdx))
		{
		...
		*A_PtrRtErrCode	=	E_GrErrNone;
		...
		// update access time
		m_CchMin.Items[Tv_CchIdx].AccTime	=	V_GrFs5Env.Tick100ms;
		
		return	(Ptr_GrFs5MinuteInfo)m_CchMin.Items[Tv_CchIdx].PtrBuf;

}
//--------------------------------------------------------------------
Def_GrErrCode	Cls_GrFs5Rgrp::LcCchMinFlushByCchIdx(__u32 A_CchIdx)
{
	// local -------------------
		Def_GrErrCode	Tv_Result;
		__u32	Tv_BlkOfs;
	// code --------------------
		...
		// save
		Tv_BlkOfs	=	E_GrFs5BlkOfsMinuteTbl + (m_CchMin.Items[A_CchIdx].DayIdx * E_GrFs5MitDaySize);

		Tv_Result	=	m_DiskTbl[m_CchMin.Items[A_CchIdx].DskIdx]->BlkWritePage(E_GrFs5BlkIdxMib, Tv_BlkOfs,
			m_CchMin.Items[A_CchIdx].PtrBuf, E_GrFs5MitDaySize);
...
//--------------------------------------------------------------------
BOOL8	Cls_GrFs5Rgrp::LcMinuteTblChkAndFix(__u32 A_DskIdx, Ptr_GrFs5MinuteInfo A_PtrTbl, BOOL8 A_IsFastChk, BOOL8* A_PtrRtIsFixed)
{
	// local -------------------
		BOOL8	Tv_Result;
		BOOL8	Tv_IsFix;
		...
		if((NULL != Tv_BiTbl) && (1 < Tv_BlkCnt))
		{
			...
			for(Tv_MnutIdx = 0; Tv_MnutIdx < E_GrTimeMinOfDay; Tv_MnutIdx++)
			{
				...
				if((0 != Tv_MnutTbl[Tv_MnutIdx].AdrMin.Idx) || ((0 != Tv_MnutTbl[Tv_MnutIdx].Adr30s.Idx)))
				{
					...
					if(0 != Tv_MnutTbl[Tv_MnutIdx].AdrMin.Idx)
					{
						...
						if(Tv_BlkCnt > Tv_MnutTbl[Tv_MnutIdx].AdrMin.Idx)
						{
							if((Tv_BiTbl[Tv_MnutTbl[Tv_MnutIdx].AdrMin.Idx].Db.OvwtId == Tv_MnutTbl[Tv_MnutIdx].AdrMin.OvwtId) &&
								(E_GrFs5BlkStatBad != Tv_BiTbl[Tv_MnutTbl[Tv_MnutIdx].AdrMin.Idx].Db.BlkStat) && 
					...
					if(0 != Tv_MnutTbl[Tv_MnutIdx].Adr30s.Idx)
					{
						...
						if(Tv_BlkCnt > Tv_MnutTbl[Tv_MnutIdx].Adr30s.Idx)
						{
							if((Tv_BiTbl[Tv_MnutTbl[Tv_MnutIdx].Adr30s.Idx].Db.OvwtId == Tv_MnutTbl[Tv_MnutIdx].Adr30s.OvwtId) &&
								(E_GrFs5BlkStatBad != Tv_BiTbl[Tv_MnutTbl[Tv_MnutIdx].Adr30s.Idx].Db.BlkStat) && 
					...
					if(Tv_IsDelMnut)
					{
						GrDumyZeroMem(&Tv_MnutTbl[Tv_MnutIdx], sizeof(St_GrFs5MinuteInfo));
					}
...
//--------------------------------------------------------------------
BOOL8	Cls_GrFs5Rgrp::LcDskDayCheckByIdx(__u32 A_DskIdx, __u32 A_DayIdx, BOOL8 A_IsFastChk, BOOL8 A_IsBadDelete)
{
	// local -------------------
		BOOL8	Tv_Result;
		BOOL8	Tv_IsFix;
...
//--------------------------------------------------------------------
Def_GrErrCode	Cls_GrFs5Rgrp::LcDskMinuteItemRead(__u32 A_DskIdx, __u32 A_TimeMinute, Ptr_GrFs5MinuteInfo A_PtrRtBuf)
{
	// local -------------------
		Def_GrErrCode	Tv_Result;
		__s32	Tv_DayIdx;
	...
	// code --------------------
		...
		// copy data
		GrDumyCopyMem(A_PtrRtBuf, &Tv_MnutTbl[Tv_MnutIdx], E_GrFs5MinuteItemSize);

		return	E_GrErrNone;

}
//--------------------------------------------------------------------
Def_GrErrCode	Cls_GrFs5Rgrp::LcDskMinuteItemWrite(__u32 A_DskIdx, __u32 A_TimeMinute, Ptr_GrFs5MinuteInfo A_PtrBuf)
{
	// local -------------------
		Def_GrErrCode	Tv_Result;
		__s32	Tv_DayIdx;
	...
	// code --------------------
		...
		// calculate minute index
		Tv_MnutIdx	=	(A_TimeMinute - Tv_TimeDay) / E_GrTimeSecOfMin;
		GrDumyCopyMem(&Tv_MnutTbl[Tv_MnutIdx], A_PtrBuf, E_GrFs5MinuteItemSize);
...
//--------------------------------------------------------------------
BOOL8	Cls_GrFs5Rgrp::LcGrpRecMinuteOpen(__u32 A_TimeMinute)
{
	// local -------------------
		BOOL8	Tv_Result;
	// code --------------------
		...
		if(0 <= m_RecDskIdx)
		{
			...
			{
				// clear
				GrDumyZeroMem(&m_RecBlkEnv.MinuteInfo, sizeof(E_GrFs5MinuteItemSize));
			}
			...
			if((0 == m_RecBlkEnv.MinuteInfo.AdrMin.Idx) && (0 == m_RecBlkEnv.MinuteInfo.Adr30s.Idx))
			{
				...
				GrDumyZeroMem(&m_RecBlkEnv.MinuteInfo, sizeof(St_GrFs5MinuteInfo));
			}
...
//--------------------------------------------------------------------
BOOL8	Cls_GrFs5Rgrp::LcGrpRecMinuteClose(void)
{
	// local -------------------
		BOOL8	Tv_Result;
	// code --------------------
...
//--------------------------------------------------------------------
Def_GrErrCode	Cls_GrFs5Rgrp::LcGrpRecMinuteFlush(void)
{
	// local -------------------
	// code --------------------
		if ((0 > m_RecDskIdx) || (0 == m_RecBlkEnv.TimeMinute))
...
//--------------------------------------------------------------------
Def_GrErrCode	Cls_GrFs5Rgrp::LcDskDayDelByIdx(__u32 A_DskIdx, __u32 A_DayIdx)
{
	// local -------------------
		Def_GrErrCode	Tv_ErrCode;
		
		Ptr_GrFs5DayInfo	Tv_DayTbl;
		//Ptr_GrFs5MinuteInfo	Tv_PtrMinTbl;
	// code --------------------
		if((E_GrFs5DayTblCntOfDisk <= A_DayIdx) && ((__u32)m_DiskCnt <= A_DskIdx))
		{
			return	E_GrErrBadParam;
		...
		// clear data
		GrDumyZeroMem(Tv_PtrMinTbl, E_GrFs5MitDaySize);
		LcCchMinAccessUpdate(A_DskIdx, A_DayIdx, TRUE);
#endif
...
//--------------------------------------------------------------------
__s32	Cls_GrFs5Rgrp::LcDskDayCreate(__u32 A_DskIdx, __u32 A_TimeDay)
{
	// local -------------------
	...
	// code --------------------
		...
		if(0 <= Tv_Result)
		{
			...
			{
				DbgMsgPrint("Cls_GrFs5Rgrp::LcDskDayCreate - disk %d day index %d data get fail!\n", A_DskIdx,Tv_Result);
				return	Tv_Result;
			}
...
//--------------------------------------------------------------------
__s32	Cls_GrFs5Rgrp::LcDskDayFind(__u32 A_DskIdx, __u32 A_TimeDay)
{
	// local -------------------
		__s32	Tv_Result;
		__u32	Tv_DayIdx;
...
//--------------------------------------------------------------------
BOOL8	Cls_GrFs5Rgrp::LcDskDayFindByTimeFw(__u32 A_DskIdx, __u32 A_Time, __s32* A_PtrRtDayIdx, __u32* A_PtrRtTime)
{
	// local -------------------
		BOOL8	Tv_Result;
		__s32	Tv_FndDay;
...
//--------------------------------------------------------------------
BOOL8	Cls_GrFs5Rgrp::LcGrpDayFindByTimeFw(__u32 A_Time, __s32* A_PtrRtDskIdx, __s32* A_PtrRtDayIdx, __u32* A_PtrRtTime)
{
	// local -------------------
		BOOL8	Tv_Result;
		__s32	Tv_FndDsk;
...
//--------------------------------------------------------------------
BOOL8	Cls_GrFs5Rgrp::LcDskDayFindByTimeBw(__u32 A_DskIdx, __u32 A_Time, __s32* A_PtrRtDayIdx, __u32* A_PtrRtTime)
{
	// local -------------------
		BOOL8	Tv_Result;
		__s32	Tv_FndDay;
...
//--------------------------------------------------------------------
BOOL8	Cls_GrFs5Rgrp::LcGrpDayFindByTimeBw(__u32 A_Time, __s32* A_PtrRtDskIdx, __s32* A_PtrRtDayIdx, __u32* A_PtrRtTime)
{
	// local -------------------
		BOOL8	Tv_Result;
		__s32	Tv_FndDsk;
...
//--------------------------------------------------------------------
BOOL8	Cls_GrFs5Rgrp::LcGrpBlkAdrFindByTimeFw(__u32 A_Time, __s32* A_PtrRtDskIdx, __s32* A_PtrRtBlkIdx, __u32* A_PtrRtSeqId, __u8* A_PtrRtOvwtId, __s32* A_PtrRtOfs)
{
	// local -------------------
		BOOL8	Tv_Result;
		BOOL8	Tv_IsFix;
...
//--------------------------------------------------------------------
BOOL8	Cls_GrFs5Rgrp::LcGrpBlkAdrFindByTimeBw(__u32 A_Time, __s32* A_PtrRtDskIdx, __s32* A_PtrRtBlkIdx, __u32* A_PtrRtSeqId, __u8* A_PtrRtOvwtId, __s32* A_PtrRtOfs)
{
	// local -------------------
		BOOL8	Tv_Result;
		BOOL8	Tv_IsFix;
...
//--------------------------------------------------------------------
BOOL8	Cls_GrFs5Rgrp::LcGrpBlkFindByTimeRngBw(__u32 A_TimeSt, __u32 A_TimeEd, __s32* A_PtrRtDskIdx, __s32* A_PtrRtBlkIdx, __u32* A_PtrRtBlkSeq, __u8* A_PtrRtOvwtId)
{
	// local -------------------
		BOOL8	Tv_Result;
		__u32	Tv_DskIdx;
...
//--------------------------------------------------------------------
BOOL8	Cls_GrFs5Rgrp::LcBlkChkSeqOverlapped(__u32 A_DskIdx, __u32 A_BlkIdx, __u32 A_SeqId)
{
	// local -------------------
		BOOL8	Tv_Result;
		__u32	Tv_DskIdx;
	...
	// code --------------------
		...
		while(Tv_DskIdx < m_DiskCnt)
		{
			...
			if((1 < Tv_BlkCnt) && (NULL != Tv_BiTbl))
			{
				...
				{
					// check exist block
					if((0 != Tv_BiTbl[Tv_BlkIdx].Db.SeqId) && (E_GrFs5BlkStatBad != Tv_BiTbl[Tv_BlkIdx].Db.BlkStat))
					{
						// check overlapped
...
//--------------------------------------------------------------------
BOOL8	Cls_GrFs5Rgrp::LcDskBlkDbErase(__u32 A_DskIdx, __u32 A_BlkIdx)
{
	// local -------------------
		BOOL8	Tv_Result;
		Ptr_GrFs5BlkInfoBase	Tv_BiTbl;
	...
	// code --------------------
		...
		if(A_DskIdx < m_DiskCnt)
		{
			...
			if((1 < Tv_BlkCnt) && (NULL != Tv_BiTbl))
			{
				if((0 < A_BlkIdx) && (A_BlkIdx < Tv_BlkCnt))
				{
					Tv_BiTbl[A_BlkIdx].Db.BlkStat			=	E_GrFs5BlkStatNormal;
...
//--------------------------------------------------------------------
BOOL8	Cls_GrFs5Rgrp::LcGrpChkBlkInfoBroken(void)
{
	// local -------------------
		BOOL8	Tv_Result;
		BOOL8	Tv_IsErase;
			...
			for( Tv_DskIdx = 0; Tv_DskIdx < m_DiskCnt; Tv_DskIdx++)
			{
				...
				if((1 < Tv_BlkCnt) && (NULL != Tv_BiTbl))
				{
					for( Tv_BlkIdx = 1; Tv_BlkIdx < Tv_BlkCnt; Tv_BlkIdx++)
					{
						...
						if(E_GrFs5BlkStatBad != Tv_BiTbl[Tv_BlkIdx].Db.BlkStat)
						{
							// init
							Tv_IsErase	=	FALSE;
							// check size zero 
							...
							{
								// check zero full
								if(E_GrFs5BlkStatFull == Tv_BiTbl[Tv_BlkIdx].Db.BlkStat)
								{
									DbgMsgPrint("Cls_GrFs5Rgrp::LcGrpChkBlkInfoBroken - zero full block found!\n");
									Tv_IsErase	=	TRUE;
								}
								// check log offset
								if(0 <= Tv_BiTbl[Tv_BlkIdx].Db.OfsLastLog)
								{
									DbgMsgPrint("Cls_GrFs5Rgrp::LcGrpChkBlkInfoBroken - bad log offset found!\n");
									Tv_IsErase	=	TRUE;
								}
								// exist sequence id
								if(E_GrFs5SeqIdNone != Tv_BiTbl[Tv_BlkIdx].Db.SeqId)
								{
									DbgMsgPrint("Cls_GrFs5Rgrp::LcGrpChkBlkInfoBroken - empty block exist sequence id!\n");
									Tv_IsErase	=	TRUE;
								}
								// check exist time range
								if((0 != Tv_BiTbl[Tv_BlkIdx].Db.TimeRng.TimeSt) || (0 != Tv_BiTbl[Tv_BlkIdx].Db.TimeRng.TimeEd))
								{
									DbgMsgPrint("Cls_GrFs5Rgrp::LcGrpChkBlkInfoBroken - empty block exist time range!\n");
									Tv_IsErase	=	TRUE;
								}
							...
							if((0 == Tv_BiTbl[Tv_BlkIdx].Db.TimeRng.TimeSt) || (0 == Tv_BiTbl[Tv_BlkIdx].Db.TimeRng.TimeEd))
							{
								DbgMsgPrint("Cls_GrFs5Rgrp::LcGrpChkBlkInfoBroken - bad time range!\n");
								Tv_IsErase	=	TRUE;
							}
							...
							if(E_GrFs5SeqIdNone == Tv_BiTbl[Tv_BlkIdx].Db.SeqId)
							{
								DbgMsgPrint("Cls_GrFs5Rgrp::LcGrpChkBlkInfoBroken - bad sequence id!\n");
								Tv_IsErase	=	TRUE;
							}
							...
							if(!Tv_Result)
							{
								...
								{
									// overlapped id found
									DbgMsgPrint("Cls_GrFs5Rgrp::LcGrpChkSeqOverlapped - overlapped sequence id found!\n");
									Tv_Result	=	TRUE;
								}
							}
						}
...
//--------------------------------------------------------------------
void	Cls_GrFs5Rgrp::LcGrpRecInfoInit(void)
{
	// local -------------------
		__s32	Tv_FndDsk;
		__s32	Tv_FndBlk;
...
//--------------------------------------------------------------------
void	Cls_GrFs5Rgrp::LcInit(void)
{
	// local -------------------
	// code --------------------
		// build disk table
		...
		{
			DbgMsgPrint("Cls_GrFs5Rgrp::LcInit - overlapped id found!\n");
...
//--------------------------------------------------------------------
BOOL8	Cls_GrFs5Rgrp::LcGrpDayRangeGet(__u32* A_PtrRtDaySt, __u32* A_PtrRtDayEd)
{
	// local -------------------
		BOOL8	Tv_Result;
		__u32	Tv_DskIdx;
...
//--------------------------------------------------------------------
void	Cls_GrFs5Rgrp::LcGrpDelSlateBlk(__u32 A_TimeBase)
{
	// local -------------------
		__u32	Tv_DskIdx;
		Ptr_GrFs5BlkInfoBase	Tv_PtrBi;
...
//--------------------------------------------------------------------
void	Cls_GrFs5Rgrp::LcGrpDelSlateTimeMap(__u32 A_TimeBase)
{
	// local -------------------
		__u32	Tv_DskIdx;
		__u32	Tv_DayIdx;
...
//--------------------------------------------------------------------
void	Cls_GrFs5Rgrp::DeleteStaleData(__u32 A_TimeBase)
{
	// local -------------------
...
//--------------------------------------------------------------------
BOOL8	Cls_GrFs5Rgrp::LcCchMinAccessUpdate(__u32 A_DskIdx, __u32 A_DayIdx, BOOL8 A_IsDirt)
{
	// local -------------------
		__s32	Tv_CchIdx;
	// code --------------------
...
//--------------------------------------------------------------------
void	Cls_GrFs5Rgrp::LcCchMinFlushAll(void)
{
	// local -------------------
		__u32	Tv_CchIdx;
	// code --------------------
...
//--------------------------------------------------------------------
__s32	Cls_GrFs5Rgrp::LcCchDataFindByHndl(__u64 A_Hndl)
{
	// local -------------------
		__u32	Tv_ItmIdx;
	// code --------------------
...
//--------------------------------------------------------------------
__s32	Cls_GrFs5Rgrp::LcCchDataFindEmpty(void)
{
	// local -------------------
		__u32	Tv_ItmIdx;
	// code --------------------
...
//--------------------------------------------------------------------
BOOL8	Cls_GrFs5Rgrp::LcCchDataChkRsvdByIdx(__u32 A_CchIdx)
{
	// local -------------------
		__u32	Tv_RsvIdx;
	// code --------------------
...
//--------------------------------------------------------------------
__s32	Cls_GrFs5Rgrp::LcCchDataFindLoadable(void)
{
	// local -------------------
		__u32	Tv_ItmIdx;
		__s32	Tv_FndIdx;
...
//--------------------------------------------------------------------
void	Cls_GrFs5Rgrp::LcCchDataFlushByIdx(__u32 A_CchIdx)
{
	// local -------------------
		__u32	Tv_WriteSize;
		Def_GrErrCode	Tv_ErrCode;
	// code --------------------
		...
		if(m_CchData.Cnt <= A_CchIdx)
		{
			DbgMsgPrint("Cls_GrFs5Rgrp::LcCchDataFlushByIdx - bad index (idx:%d, total:%d) input!\n", A_CchIdx, m_CchData.Cnt);
			return;
		}
		// check valid cache
...
//--------------------------------------------------------------------
void	Cls_GrFs5Rgrp::LcCchDataFlushAll(void)
{
	// local -------------------
		__u32	Tv_CchIdx;
	// code --------------------
...
//--------------------------------------------------------------------
void	Cls_GrFs5Rgrp::LcCchDataFreeByIdx(__u32 A_CchIdx)
{
	// local -------------------
	// code --------------------
		// check index
		if(m_CchData.Cnt <= A_CchIdx)
		{
			DbgMsgPrint("Cls_GrFs5Rgrp::LcCchDataFreeByIdx - bad index (idx:%d, total:%d) input!\n", A_CchIdx, m_CchData.Cnt);
			return;
		}
		// check valid cache
		if(E_GrDvrRecHndlNone == m_CchData.Items[A_CchIdx].Hndl)
		...
		if(0 < m_CchData.Items[A_CchIdx].Locked)
		{
			DbgMsgPrint("Cls_GrFs5Rgrp::LcCchDataFreeByIdx - cache %d locked!\n", A_CchIdx);
			return;
		}
...
//--------------------------------------------------------------------
__u32	Cls_GrFs5Rgrp::LcCchDataCalSizeByHndl(__u64 A_Hndl)
{
	// local -------------------
		__u32	Tv_Result;
	...
	// code --------------------
		...
		if(E_GrFs5RgrpCchDataBufSize <= Tv_Result)
		{
			Tv_Result	=	E_GrFs5RgrpCchDataBufSize;
		}
...
//--------------------------------------------------------------------
void	Cls_GrFs5Rgrp::LcGrpStatUpdate(void)
{
	// local -------------------
		__u8	Tv_Stat;
				...
				if(NULL != Tv_PtrBi)
				{
					for(Tv_BlkIdx = 1; Tv_BlkIdx < Tv_BlkCnt; Tv_BlkIdx++)
					{
						...
						if(E_GrFs5BlkStatBad != Tv_PtrBi[Tv_BlkIdx].Db.BlkStat)
						{
							...
							// check full block
							if(E_GrFs5BlkStatFull == Tv_PtrBi[Tv_BlkIdx].Db.BlkStat)
							{
...
//--------------------------------------------------------------------
Def_GrErrCode	Cls_GrFs5Rgrp::LcGrpReadAlignByHndl(__u64 A_Hndl, void* A_PtrBuf, __u32* A_PtrRtSize)
{
	// local -------------------
		Def_GrErrCode	Tv_ErrCode;
	...
	// code --------------------
		...
		// check offset
		if(0 != (Tv_BlkOfs & E_GrFs5RgrpCchDataOfsMask))
		{
			DbgMsgPrint("Cls_GrFs5Rgrp::LcGrpReadAlignByHndl - bad handle offset input!\n");
			return	E_GrErrBadParam;
		}
		...
		{
			DbgMsgPrint("Cls_GrFs5Rgrp::LcGrpReadAlignByHndl - bad handle disk index input!\n");
			return	E_GrErrBadParam;
		}
		...
		if(NULL == Tv_PtrBi)
		{
			DbgMsgPrint("Cls_GrFs5Rgrp::LcGrpReadAlignByHndl - block info get fail!\n");
			return	E_GrErrInitFail;
		}
		...
		if(E_GrFs5RgrpCchDataBufSize <= Tv_InSize)
		{
			Tv_InSize	=	E_GrFs5RgrpCchDataBufSize;
		}

		// patch
		Tv_ReadSize	=	(Tv_InSize + (E_GrFs5HddIoUnitSize - 1)) & E_GrFs5HddIoAdrMask;
...
//--------------------------------------------------------------------
Def_GrErrCode	Cls_GrFs5Rgrp::LcGrpWriteAlignByHndl(__u64 A_Hndl, void* A_PtrBuf, __u32* A_PtrRtSize)
{
	// local -------------------
		Def_GrErrCode	Tv_ErrCode;
	...
	// code --------------------
		...
		if(0 != (Tv_BlkOfs & E_GrFs5RgrpCchDataOfsMask ))
		{
			DbgMsgPrint("Cls_GrFs5Rgrp::LcGrpWriteAlignByHndl - bad handle offset input!\n");
			return	E_GrErrBadParam;
		}
		...
		if((__s32)m_DiskCnt <= Tv_DskIdx)
		{
			DbgMsgPrint("Cls_GrFs5Rgrp::LcGrpWriteAlignByHndl - bad handle disk index input!\n");
			return	E_GrErrBadParam;
		}
		...
		if(NULL == Tv_PtrBi)
		{
			DbgMsgPrint("Cls_GrFs5Rgrp::LcGrpWriteAlignByHndl - block info get fail!\n");
			return	E_GrErrInitFail;
		}
		...
		// patch
		Tv_WriteSize	=	(Tv_InSize + (E_GrFs5HddIoUnitSize - 1)) & E_GrFs5HddIoAdrMask;
...
//--------------------------------------------------------------------
Ptr_GrFs5DbBlkInfo	Cls_GrFs5Rgrp::LcGrpDbBlkInfoGetByHndl(__u64 A_Hndl)
{
	// local -------------------
		Ptr_GrFs5DbBlkInfo	Tv_PtrBi;
	...
	// code --------------------
		...
		// get block info
		Tv_PtrBi	=	(Ptr_GrFs5DbBlkInfo)m_DiskTbl[Tv_DskIdx]->BlkInfoTableGet(&Tv_BlkCnt);
		if(NULL == Tv_PtrBi)
		{
			return	NULL;
		}
...
//--------------------------------------------------------------------
__s32	Cls_GrFs5Rgrp::LcCchDataGetByHndl(__u64 A_Hndl, BOOL8 A_IsReadSkip, __u32* A_PtrRtInSize, Def_GrErrCode* A_PtrRtErr)
{
	// local -------------------
		__s32	Tv_ItmIdx;
		__u32	Tv_ReadSize;
	...
	// code --------------------
		...
		if (0 <= Tv_ItmIdx)
		{
			...
			*A_PtrRtInSize	=	LcCchDataCalSizeByHndl(A_Hndl);
			...
			m_CchData.Items[Tv_ItmIdx].AccTime	=	V_GrFs5Env.Tick100ms;
			return	(__s32)Tv_ItmIdx;
		}
		...
		if (A_IsReadSkip)
		{
			...
			*A_PtrRtInSize	=	0;
			...
			{
				DbgMsgPrint("Cls_GrFs5Rgrp::LcCchDataGetByHndl - block info get fail!\n");
			}
...
//--------------------------------------------------------------------
Def_GrErrCode	Cls_GrFs5Rgrp::LcCchDataGetByHndlToSctl(Ptr_GrFs5SeekCtrl A_PtrCtrl, __u64 A_Hndl)
{
	// local -------------------
		Def_GrErrCode	Tv_ErrCode;
		__s32	Tv_CchIdx;
...
//--------------------------------------------------------------------
Def_GrErrCode	Cls_GrFs5Rgrp::LcDskReadToBuf(__u32 A_DskIdx, __u32 A_BlkIdx, __u32 A_Offset, __u32 A_Size, void* A_PtrBuf)
{
	// local -------------------
		__u64	Tv_Hndl;
		__u32	Tv_CchOfs;
	...
	// code --------------------
		// check disk index
		if((m_DiskCnt <= A_DskIdx) || (0 == A_Size) || (E_GrFs5RgrpCchDataBufSize < A_Size))
		{
			return	E_GrErrBadParam;
		}

		// calculate info
		Tv_CchOfs		=	A_Offset & E_GrFs5RgrpCchDataOfsMask;
		Tv_BlkOfs		=	A_Offset & E_GrFs5RgrpCchDataAdrMask;
		Tv_HeadSize	=	A_Size;
		Tv_TailSize	=	0;
		if(E_GrFs5RgrpCchDataBufSize < (Tv_CchOfs + Tv_HeadSize))
		{
			Tv_TailSize	=	(Tv_CchOfs + Tv_HeadSize) - E_GrFs5RgrpCchDataBufSize;
		...
		// get cache
		Tv_Hndl	=	GrFs5HndlEnc((__s32)A_DskIdx, Tv_BlkOfs, (__s32)A_BlkIdx, Tv_PtrBi->OvwtId);
...
//--------------------------------------------------------------------
Def_GrErrCode	Cls_GrFs5Rgrp::LcDskWriteFromBuf(__u32 A_DskIdx, __u32 A_BlkIdx, __u32 A_Offset, __u32 A_Size, void* A_PtrBuf)
{
	// local -------------------
		__u64	Tv_Hndl;
		__u32	Tv_CchOfs;
...
//--------------------------------------------------------------------
BOOL8	Cls_GrFs5Rgrp::RtlCchDataRsvProc(void)
{
	// local -------------------
		__u8	Tv_OvwtId;
		__s32	Tv_DskIdx;
	...
	// code --------------------
		...
		GrFs5HndlDec(Tv_Hndl, &Tv_DskIdx, &Tv_BlkOfs, &Tv_BlkIdx, &Tv_OvwtId);
		...
		else
		{
			...
			if(E_GrFs5RgrpCchDataBufSize > Tv_BlkOfs)
			{
				...
				Tv_BlkOfs		=	(Tv_BlkOfs - 1) & E_GrFs5RgrpCchDataAdrMask;
			}
			else
			{
				Tv_BlkOfs	=	Tv_BlkOfs - E_GrFs5RgrpCchDataBufSize;
...
//--------------------------------------------------------------------
Def_GrErrCode	Cls_GrFs5Rgrp::SctlFree(Ptr_GrFs5SeekCtrl A_PtrCtrl)
{
	// local -------------------
	// code --------------------
		LcCchDataUnlockBySctl(A_PtrCtrl);
