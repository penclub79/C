/*
 GAUSE file system version 1 DEMUX
...
//--------------------------------------------------------------------
Cls_GrDvrFs1Dmux::Cls_GrDvrFs1Dmux( Cls_GrTaskSvr* A_TaskSvr, Cls_GrDvrFs1Mng* A_FsMng, Ptr_GrDvrFs1SeekCtrl A_PtrSeekCtrl, BOOL8 A_IsSafe ):
Cls_GrTaskCli()
{
	// local -------------------
	// code --------------------
...
//--------------------------------------------------------------------
Cls_GrDvrFs1Dmux::~Cls_GrDvrFs1Dmux()
{
	// local -------------------
	// code --------------------
...
//--------------------------------------------------------------------
BOOL8	Cls_GrDvrFs1Dmux::FrmNext( void )
{
	// local -------------------
		BOOL8	Tv_Result;
		__u8	Tv_BufIdx;
	...
	// code --------------------
		...
		// get frame size
		Tv_FrmSize	=	E_GrDvrFs1StrmAlignSize;
		if ( (Tv_Readed - Tv_Prgs ) >= sizeof(St_GrDvrAvHd) )
		{
			Tv_PtrAvHd	=	(Ptr_GrDvrAvHd)( (__u32)m_PtrSeekCtrl->Buf[Tv_BufIdx].Ptr + Tv_Prgs );
			if ( E_GrDvrFs1StrmFcc == Tv_PtrAvHd->Fcc )
			{
				Tv_FrmSize	=	E_GrDvrFs1StrmAlignMask & ( Tv_PtrAvHd->DataSize + sizeof(St_GrDvrAvHd) + E_GrDvrRecTailMarkSize + E_GrDvrFs1StrmAlignSize - 1 );
				if ( E_GrDvrFs1DmuxStrmLimite < Tv_FrmSize )
				{
					Tv_FrmSize	=	E_GrDvrFs1StrmAlignSize;
				}
			}
			...
			{
				DbgMsgPrint( "Cls_GrDvrFs1Dmux::FrmNext - bad FCC date.\n" );
			}
		}
...
//--------------------------------------------------------------------
BOOL8	Cls_GrDvrFs1Dmux::FrmPrev( void )
{
	// local -------------------
		BOOL8	Tv_Result;
		__u8	Tv_BufIdx;
	...
	// code --------------------
		...
		if ( sizeof(St_GrDvrAvHd) <= Tv_Prgs )
		{
			...
			if ( E_GrDvrRecTailMarkId == ( E_GrDvrRecTailMarkMask & Tv_Tail )  )
			{
				...
				if ( (E_GrDvrFs1DmuxStrmLimite >= Tv_EstmSize) && (0 == (Tv_EstmSize & (~E_GrDvrFs1StrmAlignMask))) )
				{
					// check size over
					if ( Tv_Prgs >= Tv_EstmSize )
					{
						...
						{
							Tv_FrmSize	=	E_GrDvrFs1StrmAlignMask & ( Tv_PtrAvHd->DataSize + sizeof(St_GrDvrAvHd) + E_GrDvrRecTailMarkSize + E_GrDvrFs1StrmAlignSize - 1 );
							if ( (Tv_EstmSize != Tv_FrmSize) || (E_GrDvrFs1DmuxStrmLimite < Tv_FrmSize) )
							{
								Tv_FrmSize	=	E_GrDvrFs1StrmAlignSize;
							}
						}
...
//--------------------------------------------------------------------
void	Cls_GrDvrFs1Dmux::SeekBegin( void )
{
	// local -------------------
	// code --------------------
		if ( PrepareLoadFw() )
...
//--------------------------------------------------------------------
void	Cls_GrDvrFs1Dmux::SeekEnd( void )
{
	// local -------------------
	// code --------------------
		if ( PrepareLoadBw() )
...
//--------------------------------------------------------------------
void	Cls_GrDvrFs1Dmux::SeekByHndlFw( __u64 A_Hndl )
{
	// local -------------------
	// code --------------------
		if ( PrepareLoadFw() )
...
//--------------------------------------------------------------------
void	Cls_GrDvrFs1Dmux::SeekByHndlBw( __u64 A_Hndl )
{
	// local -------------------
	// code --------------------
		if ( PrepareLoadBw() )
...
//--------------------------------------------------------------------
BOOL8	Cls_GrDvrFs1Dmux::SeekByMinFw( Def_GrTime A_Time )
{
	// local -------------------
		BOOL8	Tv_Result;
	// code --------------------
...
//--------------------------------------------------------------------
BOOL8	Cls_GrDvrFs1Dmux::SeekByMinBw( Def_GrTime A_Time )
{
	// local -------------------
		BOOL8	Tv_Result;
	// code --------------------
...
//--------------------------------------------------------------------
void	Cls_GrDvrFs1Dmux::LcFrmPtrUpdt( void )
{
	// local -------------------
		__u8	Tv_BufIdx;
		__u32	Tv_Prgs;
...
//--------------------------------------------------------------------
BOOL8	Cls_GrDvrFs1Dmux::PrepareLoadFw( void )
{
		return	TRUE;
}
//--------------------------------------------------------------------
void	Cls_GrDvrFs1Dmux::AfterLoadFw( void )
{

}
//--------------------------------------------------------------------
BOOL8	Cls_GrDvrFs1Dmux::PrepareLoadBw( void )
{
		return	TRUE;
}
//--------------------------------------------------------------------
void	Cls_GrDvrFs1Dmux::AfterLoadBw( void )
{

}
//--------------------------------------------------------------------
__u8	Cls_GrDvrFs1Dmux::SeekStatGet( void )
{
	// local -------------------
		__u8	Tv_Result;
	// code --------------------
