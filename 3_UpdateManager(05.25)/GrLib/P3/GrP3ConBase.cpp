...
//====================================================================
//--------------------------------------------------------------------
Cls_GrP3ConBase::Cls_GrP3ConBase( BOOL8 A_IsCusSw )
{
	// local -------------------
		__u8	Tv_VdoChCnt;
	// code --------------------
		...
		Tv_VdoChCnt		=	V_GrP3Mng->VdoMaxChCntGet();
		...
		m_PtrQuadChMap				=	V_GrP3Mng->SetupDataPtrGet( E_GrP3CfgOfsQuadMap );

		LcCusImgUpdt();			// cursor info update
}
//--------------------------------------------------------------------
Cls_GrP3ConBase::~Cls_GrP3ConBase( void )
{
	// local -------------------
	// code --------------------
...
//--------------------------------------------------------------------
Cls_GrGdib*	Cls_GrP3ConBase::GdibGet( void )
{
		return	m_GdibScr;
}
//--------------------------------------------------------------------
Cls_GrP3DoBase*	Cls_GrP3ConBase::MouseOnObjGet( void )
{
		return	m_ObjOnMse;
}
//--------------------------------------------------------------------
void	Cls_GrP3ConBase::MouseOnObjReset( void )
{
		m_ObjOnMse	=	NULL;
		m_IsMseCap	=	FALSE;
}
//--------------------------------------------------------------------
Cls_GrP3DoBase*	Cls_GrP3ConBase::FocusKeyObjGet( void )
{
		return	m_ObjFocus;
}
//--------------------------------------------------------------------
void	Cls_GrP3ConBase::FocusReset( void )
{
		m_ObjFocus	=	NULL;
}
//--------------------------------------------------------------------
void	Cls_GrP3ConBase::FocusCorrect( __u8 A_BaseLayer )
{
	// local -------------------
		St_GrP3MsgHd	Tv_Msg;
		BOOL8	Tv_IsAble;
	...
	// code --------------------
		...
		if ( !m_IsFocusCrtReq )
		{
			...
			if ( 0 != A_BaseLayer )
			{
				if ( NULL != m_ObjFocus )
				{
					Tv_Win	=	(Cls_GrP3DoWin*)m_ObjFocus->WindowGet();
			...
			if ( Tv_IsAble )
			{
				Tv_Msg.Cmd			=	E_GrP3MsgCmdConFocusCrt;
				Tv_Msg.ConIdx		=	E_GrP3ConIdxFocus;
				Tv_Msg.WinId		=	E_GrP3WinIdNone;
				Tv_Msg.ObjId		=	E_GrP3ScptIdNone;
				m_IsFocusCrtReq	=	UiMsgPost( &Tv_Msg, sizeof(Tv_Msg) );
			}
		}
}
//--------------------------------------------------------------------
BOOL8	Cls_GrP3ConBase::UiMsgPost( Ptr_GrP3MsgHd A_PtrMsg, __u32 A_Size )
{
	// local -------------------
	// code --------------------
		// patch console index
		if ( E_GrP3ConIdxFocus == A_PtrMsg->ConIdx )
		{
			A_PtrMsg->ConIdx	=	m_ConIdx;
		}
		return	V_GrP3Mng->UiMsgPost( A_PtrMsg, A_Size );
}
//--------------------------------------------------------------------
Cls_GrP3DoBase*	Cls_GrP3ConBase::FocusObjGet( void )
{
	// local -------------------
...
//--------------------------------------------------------------------
void	Cls_GrP3ConBase::FocusObjSet( Cls_GrP3DoBase* A_Obj )
{
	// local -------------------
	...
	// code --------------------
		...
		if ( m_ObjFocus != Tv_Obj )
		{
			...
			if ( NULL != Tv_Obj )
			{
				...
				if ( E_GrP3DoClassFrame == Tv_Obj->ClassIdGet() )
				{
					Tv_Obj	=	((Cls_GrP3DoFrame*)Tv_Obj)->LastFocusObjGet();
				}
				if ( NULL != Tv_Obj )
				{
...
//--------------------------------------------------------------------
void	Cls_GrP3ConBase::LcPostFocusGetMsg( void )
{
	// local -------------------
		St_GrP3MsgHd		Tv_Msg;
		Cls_GrP3DoWin*		Tv_Win;
	// code --------------------
		Tv_Win					=	(Cls_GrP3DoWin*)m_ObjFocus->WindowGet();
		Tv_Msg.Cmd			=	E_GrP3MsgCmdFocusGet;
...
//--------------------------------------------------------------------
void	Cls_GrP3ConBase::LcPostFocusLossMsg( void )
{
	// local -------------------
		St_GrP3MsgHd	Tv_Msg;
		Cls_GrP3DoWin*	Tv_Win;
	// code --------------------
		...
		if ( NULL != m_ObjFocus )
		{
			//focus out event
			Tv_Win					=	(Cls_GrP3DoWin*)m_ObjFocus->WindowGet();
			Tv_Msg.Cmd			=	E_GrP3MsgCmdFocusLoss;
			Tv_Msg.ConIdx		=	m_ConIdx;
			Tv_Msg.WinId		=	Tv_Win->WinIdGet();
			Tv_Msg.ObjId		=	E_GrP3ScptIdNone;
			if ( !m_ObjFocus->IsWindow() )
...
//--------------------------------------------------------------------
void	Cls_GrP3ConBase::PaintReq( Ptr_GrRect A_PtrRect )
{
	// local -------------------
		__u8		Tv_WkIdx;
	...
	// code --------------------
		...
		if ( (A_PtrRect->left < A_PtrRect->right) && (A_PtrRect->top < A_PtrRect->bottom) )
		{
			...
			for ( Tv_WkIdx=0;Tv_WkIdx < E_GrP3ConPaintMaxCnt;Tv_WkIdx++ )
			{
				//check exist item
			...
			if ( !Tv_IsDid )
			{
				// insert new area
				// check full
				if ( E_GrP3ConPaintMaxCnt <= m_PntSlotCnt )
				{
					// full mode
					m_PntSlotTbl[0]	=	m_ScrRect;
					m_PntSlotCnt		=	1;
					// delete other item
					for ( Tv_WkIdx=1;Tv_WkIdx < E_GrP3ConPaintMaxCnt;Tv_WkIdx++ )
					{
						m_PntSlotTbl[Tv_WkIdx].right	=	0;
					}
				}
...
//--------------------------------------------------------------------
void	Cls_GrP3ConBase::PaintAllReq( void )
{
		PaintReq( &m_ScrRect );
}
//--------------------------------------------------------------------
void	Cls_GrP3ConBase::RtlScptUpdt( void )
{
	// local -------------------
		__u8	Tv_VdoChCnt;
	// code --------------------
...
//--------------------------------------------------------------------
void	Cls_GrP3ConBase::LcProcUiMsg( Ptr_GrP3MsgHd A_PtrMsg )
{
	// local -------------------
	// code --------------------
		switch ( A_PtrMsg->Cmd )
		{
			case E_GrP3MsgCmdConInpMouse:
				LcProcMsgInpMouse( (Ptr_GrP3MsgMouse)A_PtrMsg );
				break;
			case E_GrP3MsgCmdConInpWheel:
				LcProcMsgInpWheel( (Ptr_GrP3MsgWheel)A_PtrMsg );
				break;
			case E_GrP3MsgCmdConWinClose:
				LcProcMsgWinClose( (Ptr_GrP3MsgWinClose)A_PtrMsg );
				break;
			case E_GrP3MsgCmdConInpKey:
				LcProcMsgInpKey( (Ptr_GrP3MsgKey)A_PtrMsg );
				break;
			case E_GrP3MsgCmdConFocusCrt:
				LcProcMsgFcsCrt();
				break;
			case E_GrP3MsgCmdSysStat:
				LcProcMsgSysStat( (Ptr_GrP3MsgSysStat)A_PtrMsg );
				break;
			case E_GrP3MsgCmdConLayUpClose:
				LcProcMsgLayUpClose( (Ptr_GrP3MsgLayer)A_PtrMsg );
				break;

		}
}
//--------------------------------------------------------------------
void	Cls_GrP3ConBase::LcProcMsgFcsCrt( void )
{
	// local -------------------
		__s32	Tv_LayIdx;
		__s32	Tv_WinIdx;
		Cls_GrP3DoBase*	Tv_ObjFcs;
	// code --------------------
		// check focus is null
		if ( NULL == m_ObjFocus )
		{
			Tv_ObjFcs	=	NULL;
			for ( Tv_LayIdx = E_GrP3ConLayerMaxCnt - 1;Tv_LayIdx >= 0;Tv_LayIdx -- )
			{
				for ( Tv_WinIdx=0;Tv_WinIdx < E_GrP3ConLayerWinCnt;Tv_WinIdx++ )
				{
					//check exist win
...
//--------------------------------------------------------------------
void	Cls_GrP3ConBase::LcProcMsgInpKey( Ptr_GrP3MsgKey A_PtrMsg )
{
	// local -------------------
		Cls_GrP3DoWin*	Tv_Win;
	// code --------------------
		...
		if ( NULL != m_ObjFocus )
		{
			...
			if ( m_ObjFocus->IsCanKey() )
			{
				// patch message
				Tv_Win	=	(Cls_GrP3DoWin*)m_ObjFocus->WindowGet();
				A_PtrMsg->Hd.Cmd		=	E_GrP3MsgCmdKey;
				A_PtrMsg->Hd.WinId	=	Tv_Win->WinIdGet();
				A_PtrMsg->Hd.ObjId	=	E_GrP3ScptIdNone;
				if ( !m_ObjFocus->IsWindow() )
				{
					A_PtrMsg->Hd.ObjId	=	m_ObjFocus->ScptIdGet();
				}
				RtlUiMsgProc( (Ptr_GrP3MsgHd)A_PtrMsg );		// run process
			}
			else
			{
				FocusReset();
...
//--------------------------------------------------------------------
void	Cls_GrP3ConBase::RtlUiMsgProc( Ptr_GrP3MsgHd A_PtrMsg )
{
	// local -------------------
		__u8	Tv_LayIdx;
		__u8	Tv_WinIdx;
	// code --------------------
		// do tick
		if ( E_GrP3MsgCmdTickPeriod == A_PtrMsg->Cmd )
		{
			// do timer
			LcProcTimePeriod();
		}
		...
		else if ( E_GrP3WinIdAll == A_PtrMsg->WinId )
		{
			// broad cast window
			for ( Tv_LayIdx=0;Tv_LayIdx < E_GrP3ConLayerMaxCnt;Tv_LayIdx++ )
			{
				for ( Tv_WinIdx=0;Tv_WinIdx < E_GrP3ConLayerWinCnt;Tv_WinIdx++ )
				{
					// check exist window
					if ( NULL != m_WinTbl[Tv_LayIdx][Tv_WinIdx] )
					{
						...
						if ( E_GrP3ScptIdNone != A_PtrMsg->ObjId )
						{
							m_WinTbl[Tv_LayIdx][Tv_WinIdx]->RtlUiMsgProc( A_PtrMsg );
						}
						// window message
						if ( (E_GrP3ScptIdNone == A_PtrMsg->ObjId) || (E_GrP3ScptIdAll == A_PtrMsg->ObjId) )
						{
		...
		{
			// one window message
			GrP3MngGetWinIdInfo( A_PtrMsg->WinId, &Tv_LayIdx, &Tv_WinIdx );
			// check exist window
			if ( NULL != m_WinTbl[Tv_LayIdx][Tv_WinIdx] )
			{
				// check window message
...
//--------------------------------------------------------------------
__u32	Cls_GrP3ConBase::GetResX( void )
{
		return	m_ScrRect.right - m_ScrRect.left;
}
//--------------------------------------------------------------------
__u32	Cls_GrP3ConBase::GetResY( void )
{
		return	m_ScrRect.bottom - m_ScrRect.top;
}
//--------------------------------------------------------------------
void	Cls_GrP3ConBase::MouseCusOnOff( BOOL8 A_IsOn )
{
	// local -------------------
		BOOL8	Tv_IsOld;
		__u16	Tv_X;
	...
	// code --------------------
		...
		if ( m_IsMseCusOn )
		{
			...
			if ( !Tv_IsOld )
			{
				...
				//get mouse position
				//V_GrP3Mng->MousePosGet( &Tv_X, &Tv_Y );
...
//--------------------------------------------------------------------
void	Cls_GrP3ConBase::MouseCusPosSet( __s32 A_X, __s32 A_Y )
{
	// local -------------------
		St_GrRect	Tv_RcPnt;
	// code --------------------
		// soft cursor draw
		if ( m_IsCusSw && m_IsMseCusOn && (E_GrP3ImgIdNone != m_CusImgId) )
		{
			// double buffering
...
//--------------------------------------------------------------------
BOOL8	Cls_GrP3ConBase::HalPntReqTransfer( Ptr_GrRect A_PtrRect )
{
		return	FALSE;
}
//--------------------------------------------------------------------
void	Cls_GrP3ConBase::HalMouseCusOnOff( void )
{
		// hardware implement
}
//--------------------------------------------------------------------
void	Cls_GrP3ConBase::LcProcMsgInpMouse( Ptr_GrP3MsgMouse A_PtrMsg )
{
	// local -------------------
		Cls_GrP3DoWin*	Tv_Win;
	// code --------------------
		...
		if ( NULL != m_ObjOnMse )
		{
			Tv_Win	=	(Cls_GrP3DoWin*)m_ObjOnMse->WindowGet();
			A_PtrMsg->Hd.Cmd		=	E_GrP3MsgCmdMouse;
			A_PtrMsg->Hd.WinId	=	Tv_Win->WinIdGet();
			A_PtrMsg->Hd.ObjId	=	E_GrP3ScptIdNone;
			if ( !m_ObjOnMse->IsWindow() )
			{
				A_PtrMsg->Hd.ObjId	=	m_ObjOnMse->ScptIdGet();
			}
			RtlUiMsgProc( (Ptr_GrP3MsgHd)A_PtrMsg );
		}
}
//--------------------------------------------------------------------
void	Cls_GrP3ConBase::LcProcMsgInpWheel( Ptr_GrP3MsgWheel A_PtrMsg )
{
	// local -------------------
		Cls_GrP3DoWin*	Tv_Win;
	// code --------------------
		...
		{
			Tv_Win	=	(Cls_GrP3DoWin*)m_ObjOnMse->WindowGet();
			A_PtrMsg->Hd.Cmd		=	E_GrP3MsgCmdWheel;
			A_PtrMsg->Hd.WinId	=	Tv_Win->WinIdGet();
			A_PtrMsg->Hd.ObjId	=	E_GrP3ScptIdNone;
			if ( !m_ObjOnMse->IsWindow() )
			{
...
//--------------------------------------------------------------------
Ptr_GrP3ConPntStackItm	Cls_GrP3ConBase::PaintStackGetPushPtr( void )
{
	// local -------------------
		Ptr_GrP3ConPntStackItm	Tv_Result;
	// code --------------------
		Tv_Result	=	NULL;
		if ( m_IsPstCanPush )
		{
			// check stack full
			if ( E_GrP3ConPntStackCnt > m_PntSp )
			{
				Tv_Result	=	&m_PntStack[m_PntSp];
			}
			else
			{
				DbgMsgPrint("Cls_GrP3ConBase::PaintStackGetPushPtr - paint stack is full!\n");
			}
		}
		return	Tv_Result;
}
//--------------------------------------------------------------------
void	Cls_GrP3ConBase::PaintStackPush( void )
{
	// local -------------------
	// code --------------------
		if ( m_IsPstCanPush )
		{
			if ( E_GrP3ConPntStackCnt > m_PntSp )
			{
				...
				if (	(E_GrP3ScptPstTypeClrFill == m_PntStack[m_PntSp].Type) || 
							( (E_GrP3ScptPstTypeImage == m_PntStack[m_PntSp].Type) && (1 == m_PntStack[m_PntSp].Para0) )	
						)
				{
			...
			{
				DbgMsgPrint("Cls_GrP3ConBase::PaintStackPush - paint stack is already full!\n");
			}
		}
}
//--------------------------------------------------------------------
BOOL8	Cls_GrP3ConBase::IsPstCanPush( void )
{
		return	m_IsPstCanPush;
}
//--------------------------------------------------------------------
void	Cls_GrP3ConBase::PstPushBlock( void )
{
		m_IsPstCanPush	=	FALSE;
}
//--------------------------------------------------------------------
void	Cls_GrP3ConBase::LcSoftCusPaint( Ptr_GrRect A_PtrRect )
{
	// local -------------------
		St_GrRect	Tv_RcCus;
		Ptr_GrP3ConPntStackItm	Tv_PtrItm;
	// code --------------------
		// check area
		...
		if ( (Tv_RcCus.left < Tv_RcCus.right) && (Tv_RcCus.top < Tv_RcCus.bottom) )
		{
			...
			if ( NULL != Tv_PtrItm )
			{
				...
				Tv_PtrItm->Type		=	E_GrP3ScptPstTypeImage;
				Tv_PtrItm->Para0	=	E_GrP3ScptPstImgAtbSprite;		// sprite attribute
				Tv_PtrItm->Para1	=	0;
...
//--------------------------------------------------------------------
void	Cls_GrP3ConBase::LcPaintByRect( Ptr_GrRect A_PtrRect )
{
	// local -------------------
		__u8	Tv_LayIdx;
		__u8	Tv_WinIdx;
	// code --------------------
...
#endif
		// soft cursor
		if ( m_IsCusSw && m_IsMseCusOn && (NULL == m_GdibDbl) && (E_GrP3ImgIdNone != m_CusImgId) )
		{
		...
		Tv_LayIdx	=	E_GrP3ConLayerMaxCnt;
		while (0 != Tv_LayIdx)
		{
			Tv_LayIdx --;
			// do paint window
		...
		if ( 0 != m_PntSp )
		{
			DbgMsgPrint( "Cls_GrP3ConBase::LcPaintByRect - paint stack remained.\n" );
		}
#endif
}
//--------------------------------------------------------------------
void	Cls_GrP3ConBase::LcPaintBySlot( void )
{
	// local -------------------
		__u32	Tv_SlotIdx;
	// code --------------------
		...
		if ( (NULL != m_GdibScr) && (0 != m_PntSlotCnt) )
		{
			// find possible item
			Tv_SlotIdx	=	0;
			while ( (E_GrP3ConPaintMaxCnt > Tv_SlotIdx) && (0 != m_PntSlotCnt) )
			{
				// check exist item
			...
			if ( 0 != m_PntSlotCnt )
			{
				DbgMsgPrint("Cls_GrP3ConBase::LcPaintBySlot - paint slot count ( %d ) mismatch\n", m_PntSlotCnt );
				m_PntSlotCnt	=	0;
			}
...
//--------------------------------------------------------------------
Cls_GrP3DoBase*	Cls_GrP3ConBase::ObjFindMseByPos( __s32 A_X, __s32 A_Y )
{
	// local -------------------
		Cls_GrP3DoBase*	Tv_Result;
		BOOL8	Tv_IsLayLock;
		...
		for ( Tv_LayIdx=E_GrP3ConLayerMaxCnt-1;Tv_LayIdx >= 0;Tv_LayIdx-- )
		{
			...
			for ( Tv_WinIdx=0;Tv_WinIdx < E_GrP3ConLayerWinCnt;Tv_WinIdx++  )
			{
				...
				if ( NULL != m_WinTbl[Tv_LayIdx][Tv_WinIdx] )
				{
					...
					{
						Tv_IsLayLock	=	TRUE;
						Tv_Result			=	(Cls_GrP3DoBase*)Tv_Win;
						break;
					}
				}
			}
...
//--------------------------------------------------------------------
Cls_GrP3DoBase*	Cls_GrP3ConBase::ObjFindByPos( __s32 A_X, __s32 A_Y )
{
	// local -------------------
		Cls_GrP3DoBase*	Tv_Result;
		__s32		Tv_LayIdx;
...
//--------------------------------------------------------------------
__u32	Cls_GrP3ConBase::ObjListByPos( __s32 A_X, __s32 A_Y, __u32* A_PtrBuf )
{
	// local -------------------
		__u32	Tv_Result;
		__s32		Tv_LayIdx;
		...
		for ( Tv_LayIdx=E_GrP3ConLayerMaxCnt-1;Tv_LayIdx >= 0;Tv_LayIdx-- )
		{
			...
			for ( Tv_WinIdx=0;Tv_WinIdx < E_GrP3ConLayerWinCnt;Tv_WinIdx++  )
			{
				...
				if ( NULL != m_WinTbl[Tv_LayIdx][Tv_WinIdx] )
				{
					Tv_Win	=	m_WinTbl[Tv_LayIdx][Tv_WinIdx];
					// find positino item
					Tv_Win->RtlListObjByPos( A_X, A_Y, 0, 0, &Tv_Result, A_PtrBuf );
				}
			}
		}
		return	Tv_Result;
...
//--------------------------------------------------------------------
void	Cls_GrP3ConBase::LcMseObjUpdt( __s32 A_X, __s32 A_Y )
{
	// local -------------------
		Cls_GrP3DoBase*	Tv_Obj;
		Cls_GrP3DoWin*	Tv_Win;
		...
		if ( (!m_IsMseCap) || (NULL == m_ObjOnMse) )
		{
			...
			if ( Tv_Obj != m_ObjOnMse )
			{
				...
				if ( NULL != m_ObjOnMse )
				{
					...
					// leave focus message 
					Tv_Win	=	(Cls_GrP3DoWin*)m_ObjOnMse->WindowGet();
					Tv_Msg.Cmd		=	E_GrP3MsgCmdMouseEnter;
					Tv_Msg.ConIdx	=	E_GrP3ConIdxFocus;
					Tv_Msg.WinId	=	Tv_Win->WinIdGet();
					Tv_Msg.ObjId	=	E_GrP3ScptIdNone;
					if ( !m_ObjOnMse->IsWindow() )
...
//--------------------------------------------------------------------
void	Cls_GrP3ConBase::LcMseObjFree( void )
{
	// local -------------------
		Cls_GrP3DoWin*	Tv_Win;
		St_GrP3MsgHd		Tv_Msg;
	// code --------------------
		if ( NULL != m_ObjOnMse )
		{
			// leave focus message 
			Tv_Win	=	(Cls_GrP3DoWin*)m_ObjOnMse->WindowGet();
			Tv_Msg.Cmd		=	E_GrP3MsgCmdMouseLeave;
			Tv_Msg.ConIdx	=	E_GrP3ConIdxFocus;
			Tv_Msg.WinId	=	Tv_Win->WinIdGet();
			Tv_Msg.ObjId	=	E_GrP3ScptIdNone;
			if ( !m_ObjOnMse->IsWindow() )
...
//--------------------------------------------------------------------
void	Cls_GrP3ConBase::LcMseHideUpdt( void )
{
	// local -------------------
	// code --------------------
		...
		{
			m_MseSusTime ++;
			if ( E_GrP3ConMseSusTimeOut < m_MseSusTime )
			{
				MouseCusOnOff( FALSE );
				m_MseSusTime	=	0;
			}
...
//--------------------------------------------------------------------
void	Cls_GrP3ConBase::LcProcTimePeriod( void )
{
	// local -------------------
	// code --------------------
		// update paint
...
//--------------------------------------------------------------------
#ifdef GR_P3_EDITOR
void	Cls_GrP3ConBase::DvlDispSetup( void )
{
	// local -------------------
	// code --------------------
...
//--------------------------------------------------------------------
#ifdef GR_P3_EDITOR
Cls_GrGdib*	Cls_GrP3ConBase::DvlGdibOutGet( void )
{
		return	NULL;
}
#endif
//--------------------------------------------------------------------
void	Cls_GrP3ConBase::IndexSet( __u8 A_Idx )
{
		m_ConIdx	=	A_Idx;
}
//--------------------------------------------------------------------
__u8	Cls_GrP3ConBase::IndexGet( void )
{
		return	m_ConIdx;
}
//--------------------------------------------------------------------
void	Cls_GrP3ConBase::WindowAllFree( void )
{
	// local -------------------
		__u8	Tv_LayIdx;
		__u8	Tv_WkIdx;
	...
	// code --------------------
		for ( Tv_LayIdx=0;Tv_LayIdx < E_GrP3ConLayerMaxCnt;Tv_LayIdx++ )
		{
			for (Tv_WkIdx=0;Tv_WkIdx < E_GrP3ConLayerWinCnt;Tv_WkIdx++)
			{
				Tv_Layer	=	E_GrP3ConLayerMaxCnt - Tv_LayIdx - 1;
...
//--------------------------------------------------------------------
void	Cls_GrP3ConBase::WindowFree( __u16 A_WinId )
{
	// local -------------------
		__u16	Tv_Layer;
		__u16	Tv_Slot;
...
//--------------------------------------------------------------------
__u16	Cls_GrP3ConBase::RtlWinRegist( Cls_GrP3DoWin* A_Win, __u8 A_Layer )
{
	// local -------------------
		__u16	Tv_Result;
		__u16	Tv_SlotIdx;
	// code --------------------
		Tv_Result	=	E_GrP3WinIdNone;
		if ( E_GrP3ConLayerMaxCnt > A_Layer )
		{
			//find empty slot
			for (Tv_SlotIdx=0;Tv_SlotIdx < E_GrP3ConLayerWinCnt;Tv_SlotIdx++)
			{
				if ( NULL == m_WinTbl[A_Layer][Tv_SlotIdx] )
				{
					...
					// make window id
					m_WinSeq ++;
					Tv_Result	=	(__u16)m_WinSeq | (((__u16)A_Layer) << E_GrP3WinIdLayerBitPos) | (Tv_SlotIdx << E_GrP3WinIdSlotBitPos);
					break;
				}
			}
			if ( E_GrP3WinIdNone == Tv_Result )
			{
				DbgMsgPrint( "Cls_GrP3ConBase::RtlWinRegist - layer is full!\n" );
			}
		}
		else
		{
			DbgMsgPrint("Cls_GrP3ConBase::RtlWinRegist - bad layer ( %d ) requested!\n", (__s32)A_Layer );
		}
		return	Tv_Result;
}
//--------------------------------------------------------------------
void	Cls_GrP3ConBase::RtlAsmStatEvt( __u8 A_Stat, __u8 A_BpIdx, __u8 A_VmIdx )
{

}
//--------------------------------------------------------------------
BOOL8	Cls_GrP3ConBase::LcFindWinByWinId( __u16 A_WinId, __u16* A_PtrRtLayer, __u16* A_PtrRtSlot )
{
	// local -------------------
		BOOL8	Tv_Result;
		__u16	Tv_Layer;
	...
	// code --------------------
		Tv_Result	=	FALSE;
		// check window id
		Tv_Layer	=	(A_WinId >> E_GrP3WinIdLayerBitPos) & E_GrP3WinIdLayerBitMask;
		Tv_Slot		=	(A_WinId >> E_GrP3WinIdSlotBitPos) & E_GrP3WinIdSlotBitMask;
...
//--------------------------------------------------------------------
Cls_GrP3DoWin*	Cls_GrP3ConBase::WinFindByWinId( __u16 A_WinId )
{
	// local -------------------
		Cls_GrP3DoWin*	Tv_Result;
		__u16	Tv_Layer;
		__u16	Tv_Slot;
	// code --------------------
		Tv_Result	=	NULL;
...
//--------------------------------------------------------------------
Cls_GrP3DoWin*	Cls_GrP3ConBase::WinFindByScptId( __u16 A_ScptId )
{
	// local -------------------
		Cls_GrP3DoWin*	Tv_Result;
		__u16	Tv_Layer;
	...
	// code --------------------
		Tv_Result	=	NULL;
		for ( Tv_Layer=0;Tv_Layer < E_GrP3ConLayerMaxCnt;Tv_Layer++ )
		{
			for ( Tv_Slot=0;Tv_Slot < E_GrP3ConLayerWinCnt;Tv_Slot++ )
			{
				if ( NULL != m_WinTbl[Tv_Layer][Tv_Slot] )
				{
...
//--------------------------------------------------------------------
void	Cls_GrP3ConBase::LcDoWinClose( __u16 A_Layer, __u16 A_Slot )
{
	// local -------------------
		BOOL8	Tv_IsModal;
		__s32		Tv_ModalRslt;
	// code --------------------
		...
		{
			// get info
			Tv_IsModal		=	m_WinTbl[A_Layer][A_Slot]->IsDoingModal();
			Tv_ModalRslt	=	m_WinTbl[A_Layer][A_Slot]->ModalResultGet();
			// release
			...
			if ( Tv_IsModal )
			{
				V_GrP3Mng->VmModalPop( Tv_ModalRslt );
			}
		}
}
//--------------------------------------------------------------------
void	Cls_GrP3ConBase::LcProcMsgWinClose( Ptr_GrP3MsgWinClose A_PtrMsg )
{
	// local -------------------
		__u16	Tv_Layer;
		__u16	Tv_Slot;
...
//--------------------------------------------------------------------
void	Cls_GrP3ConBase::LcProcMsgLayUpClose( Ptr_GrP3MsgLayer A_PtrMsg )
{
	// local -------------------
		__u8	Tv_Layer;
		__u16	Tv_Slot;
	// code --------------------
		...
		while ( E_GrP3ConLayerMaxCnt > Tv_Layer ) 
		{
			for ( Tv_Slot=0;Tv_Slot < E_GrP3ConLayerWinCnt;Tv_Slot++ )
			{
...
//--------------------------------------------------------------------
void	Cls_GrP3ConBase::LcDoPstLine( void )
{
	// local -------------------
		__s32		Tv_StX;
		__s32		Tv_StY;
...
//--------------------------------------------------------------------
void	Cls_GrP3ConBase::LcDoPstImage( void )
{
	// local -------------------
	...
	// code --------------------
		// check exist image
		Tv_PtrGdib	=	V_GrP3Mng->ImgGet( (__u16)m_PntStack[m_PntSp].Value );
		if ( NULL != Tv_PtrGdib )
		{
			...
			if ( GrDrawCnGetComRect( &Tv_RcClip, &Tv_RcOrg, &m_PntStack[m_PntSp].Rect ) )
			{
				...
				// set srite mode
				Tv_IsSpr	=	FALSE;
				if ( 0 != (E_GrP3ScptPstImgAtbSprite & m_PntStack[m_PntSp].Para0) )
				{
					Tv_IsSpr	=	TRUE;
				}
				m_GdibScr->SpriteModeSet( Tv_IsSpr );
				// check fill mode
				if ( 0 != (E_GrP3ScptPstImgAtbFill & m_PntStack[m_PntSp].Para0) )
				{
					// fill mode
...
//--------------------------------------------------------------------
void	Cls_GrP3ConBase::LcDoPstFont( void )
{
	// local -------------------
	// code --------------------
		m_PntFont	=	V_GrP3Mng->FontGet( m_PntStack[m_PntSp].Para0 );
		m_PntTxtClrShadow	=	m_PntStack[m_PntSp].Value;
}
//--------------------------------------------------------------------
void	Cls_GrP3ConBase::LcDoPstStyle( void )
{
	// local -------------------
		__u8*	Tv_PtrBt;
	// code --------------------
		m_PntTxtGapPitch	=	(__s32)( (signed __s8)m_PntStack[m_PntSp].Para0 );
		m_PntTxtGapLine		=	(__s32)( (signed __s8)m_PntStack[m_PntSp].Para1 );
		...
		if ( 0 != (E_GrP3ScptPstTxtAtbDrawShadow & m_PntTxtAtb) )
		{
			m_IsPntTxtShadow	=	TRUE;
		}
}
//--------------------------------------------------------------------
void	Cls_GrP3ConBase::LcDoPstText( void )
{
	// local -------------------
	// code --------------------
		// check exist font
...
//--------------------------------------------------------------------
void	Cls_GrP3ConBase::LcDoPaintStack( Ptr_GrRect A_PtrRect )
{
	// local -------------------
	// code --------------------
		...
		if ( 0 != m_PntSp )
		{
			...
			while ( 0 != m_PntSp )
			{
				...
				{
					case E_GrP3ScptPstTypeClrFill:
						m_GdibScr->FillRect( &m_PntStack[m_PntSp].Rect, m_PntStack[m_PntSp].Value );
						break;
					case E_GrP3ScptPstTypeLine:
						LcDoPstLine();
						break;
					case E_GrP3ScptPstTypeImage:
						LcDoPstImage();
						break;
					case E_GrP3ScptPstTypeText:
						LcDoPstText();
						break;
					case E_GrP3ScptPstTypeFont:
						LcDoPstFont();
						break;
					case E_GrP3ScptPstTypeStyle:
						LcDoPstStyle();
						break;
				}
			}
...
//--------------------------------------------------------------------
void	Cls_GrP3ConBase::LcPntTxtParsProc( void )
{
	// local -------------------
		WCHAR	Tv_Code;
		__s32		Tv_SizeX;
		...
		if ( m_IsPntTxtProc )
		{
			...
			if ( 0 == Tv_Code )
			{
				...
				if ( m_PntTxtCus ==  m_PntTxtIdx)
				{
					...
					Tv_SizeX	=	m_PntFont->GetWidth() >> E_GrP3ConCusShift;
					// check end 
					if ( (m_PntTxtLineTbl[m_PntTxtLnCnt].SzX + Tv_SizeX) > (m_PntTxtRc.right - m_PntTxtRc.left)  )
					{
						// do word break
						if ( (0 != (E_GrP3ScptPstTxtAtbMultiLine & m_PntTxtAtb)) && (0 != (E_GrP3ScptPstTxtAtbWarp & m_PntTxtAtb)) )
						{
							// move next line able
							if (	(E_GrP3ConPntTxtLineMax > m_PntTxtLnCnt) &&
			...
			else 
			{
				...
				if (	(0x0A == Tv_Code) || ( (0x3B == Tv_Code) && (0 != (E_GrP3ScptPstTxtAtbSemiEnter & m_PntTxtAtb)) ) ||
							(0x0D == Tv_Code) || (0x20 == Tv_Code)
						)
				{
					Tv_SizeX	=	m_PntFont->GetWidth() >> E_GrP3ConCusShift;		// display size
				}
				else
				{
					// normal text
					// check password
					if ( 0 != (E_GrP3ScptPstTxtAtbPasswd & m_PntTxtAtb) )
					{
						Tv_Code	=	0x2A;
					}
					// get font info
					if ( m_PntFont->GetCharInfo( Tv_Code, &Tv_FntIdx, &Tv_RcFnt, &Tv_FntPitch, &Tv_FntGap ) )
					{
						...
						if ( 0 != (E_GrP3ScptPstTxtAtbFixPitch & m_PntTxtAtb) )
						{
				...
				if ( (0x0A == Tv_Code) || ( (0x3B == Tv_Code) && (0 != (E_GrP3ScptPstTxtAtbSemiEnter & m_PntTxtAtb)) ) )
				{
					...
					{
						// check multi line
						if (0 != (E_GrP3ScptPstTxtAtbMultiLine & m_PntTxtAtb))
						{
							// do next line
							// move next line able
							if (	(E_GrP3ConPntTxtLineMax > m_PntTxtLnCnt) &&
...
//--------------------------------------------------------------------
void	Cls_GrP3ConBase::LcPntTxtAreaBuild( void )
{
	// local -------------------
		__u8		Tv_WkIdx;
	// code --------------------
		...
		if ( m_PntTxtAreaY <= (m_PntTxtRc.bottom - m_PntTxtRc.top) )
		{
			...
			{
				// calculate align y
				if ( E_GrP3ScptPstTxtAlignVtTop == m_PntTxtAlignVt )
				{
					m_PntTxtStY	=	m_PntTxtRc.top;
				}
				else if ( E_GrP3ScptPstTxtAlignVtBottom == m_PntTxtAlignVt )
				{
				...
				{
					if ( E_GrP3ScptPstTxtAlignHzLeft == m_PntTxtAlignHz )
					{
						m_PntTxtLineTbl[Tv_WkIdx].StX	=	m_PntTxtRc.left;
					}
					else if ( E_GrP3ScptPstTxtAlignHzRight == m_PntTxtAlignHz )
					{
...
//--------------------------------------------------------------------
void	Cls_GrP3ConBase::LcPntTxtDraw( void )
{
	// local -------------------
		WCHAR	Tv_Code;
		__u8	Tv_LnIdx;
				...
				if ( 0 != m_PntTxtLineTbl[Tv_LnIdx].Cnt )
				{
					...
					for ( Tv_CdIdx=0;Tv_CdIdx < m_PntTxtLineTbl[Tv_LnIdx].Cnt;Tv_CdIdx ++ )
					{
						...
						if (	(0x00 == Tv_Code) ||	(0x0A == Tv_Code) || 
									( (0x3B == Tv_Code) && (0 != (E_GrP3ScptPstTxtAtbSemiEnter & m_PntTxtAtb)) ) ||
									(0x0D == Tv_Code) || (0x20 == Tv_Code)
							)
						{
							Tv_SizeX	=	m_PntFont->GetWidth() >> E_GrP3ConCusShift;		// display size
						...
						if ( Tv_IsFont )
						{
							...
							if ( m_PntTxtCus == m_PntTxtIdx )
							{
								Tv_ClrTxt	=	V_GrP3Mng->CursorColorGet();
							}
						...
						else
						{
							...
							if ( m_PntTxtCus == m_PntTxtIdx )
							{
								...
								m_GdibScr->FillRect( &Tv_RcFnt, V_GrP3Mng->CursorColorGet() );
							}
						}
						// next
...
//--------------------------------------------------------------------
__u8 Cls_GrP3ConBase::QuadPageCntByMode( __u8 A_Mode )
{
	// local -------------------
		__u8	Tv_Result;
		__u8	Tv_MaxChCnt;
		__u8	Tv_ViewCnt;
	// code --------------------
		Tv_MaxChCnt	=	V_GrP3Mng->VdoMaxChCntGet();
...
//--------------------------------------------------------------------
BOOL8	Cls_GrP3ConBase::QuadModeSet( __u8 A_Mode, __u8 A_Page, __u32 A_Fcc )
{
	// local -------------------
		BOOL8	Tv_Result;
		__u8	Tv_PageMax;
...
//--------------------------------------------------------------------
void	Cls_GrP3ConBase::QuadModeNext( __u32 A_Fcc )
{
	// local -------------------
		__u8	Tv_WkIdx;
		__u8	Tv_Mode;
...
//--------------------------------------------------------------------
void	Cls_GrP3ConBase::QuadModePrev( __u32 A_Fcc )
{
	// local -------------------
		__u8	Tv_WkIdx;
		__u8	Tv_Mode;
...
//--------------------------------------------------------------------
__u8	Cls_GrP3ConBase::QuadGridTblBuild( Ptr_GrRect A_PtrRtBuf, __u8 A_Width )
{
		return	0;
}
//--------------------------------------------------------------------
__u8	Cls_GrP3ConBase::QuadViewRectBuild( Ptr_GrRect A_PtrRtBuf )
{
		return	0;
}
//--------------------------------------------------------------------
__u8*	Cls_GrP3ConBase::QuadViewChMapPtrGet( void )
{
		return	GrDvrQuadMapPtr( m_PtrQuadChMap , m_QuadMode, m_QuadPage );
}
//--------------------------------------------------------------------
#ifdef GR_P3_EDITOR
void	Cls_GrP3ConBase::DvlDebugPrepare( void )
{
	// local -------------------
		__u8	Tv_VdoChCnt;
	// code --------------------
...
#endif
//--------------------------------------------------------------------
BOOL8	Cls_GrP3ConBase::IsShutDown( void )
{
		return	m_IsShutDown;
}
//--------------------------------------------------------------------
void	Cls_GrP3ConBase::LcCusImgUpdt( void )
{
	// local -------------------
		__u16	Tv_ImgId;
	// code --------------------
		...
		Tv_ImgId		=	V_GrP3Mng->SoftCusImgIdGet();
		if ( E_GrP3ImgIdNone != Tv_ImgId )
		{
			m_PtrGdibCus	=	V_GrP3Mng->ImgGet( Tv_ImgId );
...
//--------------------------------------------------------------------
void	Cls_GrP3ConBase::HalMousePosSet( __s32 A_X, __s32 A_Y )
{

}
//--------------------------------------------------------------------
__u16	Cls_GrP3ConBase::QuadDispChMapGet( void )
{
	// local -------------------
		__u16	Tv_Result;
		__u8*	Tv_PtrChMap;
...
//--------------------------------------------------------------------
void	Cls_GrP3ConBase::LcProcMsgSysStat( Ptr_GrP3MsgSysStat A_PtrMsg )
{
	// local -------------------
	...
	// code --------------------
		if ( E_GrP3SysStatQuadMode == A_PtrMsg->Type )
		{
			...
			if ( E_GrP3AsmIdNone != Tv_PtrEvt[m_ConIdx] )
			{
				V_GrP3Mng->VmRunCode( Tv_PtrEvt[m_ConIdx], NULL, NULL );
			}
		}
}
//--------------------------------------------------------------------
void	Cls_GrP3ConBase::QuadModeGet( __u8* A_PtrRtMode, __u8* A_PtrRtPage )
{
	// local -------------------
	// code --------------------
...
//--------------------------------------------------------------------
void	Cls_GrP3ConBase::QuadPageNext( __u32 A_Fcc )
{
	// local -------------------
		__u8		Tv_QuadPage;
		__u8		Tv_MaxPage;
...
//--------------------------------------------------------------------
void	Cls_GrP3ConBase::HalMouseCusImgSet( Ptr_GrGdib A_PtrGdib )
{

}
//--------------------------------------------------------------------
void	Cls_GrP3ConBase::MouseCapture( void )
{
	// local -------------------
	// code --------------------
		m_IsMseCap	=	TRUE;
...
//--------------------------------------------------------------------
void	Cls_GrP3ConBase::MouseRelease( void )
{
		m_IsMseCap	=	FALSE;
}
//--------------------------------------------------------------------
