...
//--------------------------------------------------------------------
BOOL8	Cls_GrDvrFs3Disk::LcIovWrite( struct iovec* A_PtrIov, __s32 A_IovCnt, __u32 A_TotSize )
{
	// local -------------------
		BOOL8	Tv_Result;
#ifndef LINUX_APP
	...
	// code --------------------
		...
		if ( -1 != m_HndlDsk )
		{
			if ( (__s32)A_TotSize == writev( m_HndlDsk, A_PtrIov, A_IovCnt ) )
			{
				Tv_Result	=	TRUE;
			}
			/*
		...
		if ( INVALID_HANDLE_VALUE != m_HndlDsk )
		{
			if ( 0 != A_IovCnt )
			{
				...
				{
					if ( !WriteFile(m_HndlDsk, A_PtrIov[Tv_WkIdx].iov_base, A_PtrIov[Tv_WkIdx].iov_len, &Tv_Writed, NULL) )
					{
						Tv_Result		=	FALSE;
					}
				}
...
//--------------------------------------------------------------------
BOOL8	Cls_GrDvrFs3Disk::BlockDataAdd( __u32 A_BlkId, __u32 A_RecId, struct iovec* A_PtrIov, __s32 A_IovCnt, __u32 A_TotSize,
				...
				if ( 0 != A_TotSize )
				{
					if ( !m_Bdt[A_BlkId].IsBad  )
					{
						if (!m_Bdt[A_BlkId].IsFull)
						{
							...
							if ( Tv_FreeSize >= A_TotSize )
							{
								// write data
								if ( LcBlockWrite( A_BlkId, m_Bdt[A_BlkId].RecSize, A_PtrIov, A_IovCnt, A_TotSize, TRUE ) )
								{
									// update info
									m_Bdt[A_BlkId].RecSize	=	m_Bdt[A_BlkId].RecSize + A_TotSize;
									// check valid time
...
//--------------------------------------------------------------------
BOOL8	Cls_GrDvrFs3Disk::LcBlockWrite( __u32 A_BlkId, __u32 A_Ofs, struct iovec* A_PtrIov, __s32 A_IovCnt, __u32 A_TotSize, BOOL8 A_IsLhndlUpdt )
{
	// local -------------------
		BOOL8	Tv_Result;
		__u64	Tv_RdaPos;
	...
	// code --------------------
		...
		if ( LcSeek( Tv_Dpos ) )
		{
			if ( LcIovWrite( A_PtrIov, A_IovCnt, A_TotSize ) )
			{
				Tv_Result	=	TRUE;
				// update last handle
				if ( A_IsLhndlUpdt )
