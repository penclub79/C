																 ...
																 Ptr_GrBufTsMng A_RecBufMng, __u16 A_RecOwnerId, __u16 A_RecThrdId )
{
	...
	// code --------------------
		...
		m_RecBufMng		=	A_RecBufMng;
		m_RecOwnerId	=	A_RecOwnerId;
		m_RecThrdId		=	A_RecThrdId;
...
//--------------------------------------------------------------------
BOOL8	Cls_GrDvrFs3Grp::StreamAdd( __u8 A_Ch, __u8 A_Type, __u8 A_Evt, __u8 A_Res, __u8 A_SkipCnt, __u8 A_SeqId, 
			Def_GrTime A_TimeSec, __u32 A_TimeMili, BOOL8 A_IsSm, void* A_PtrBuf, __u32	A_StrmSize )
{
	...
	// code --------------------
		...
		if ( (0 != m_DiskCnt) && m_IsRecAble )
		{
			if (0 != A_Evt)
			{
				Tv_Result	=	LcGthPktAdd( A_Ch, A_Type, A_Evt, A_Res, A_SkipCnt, A_SeqId, A_TimeSec, A_TimeMili, A_IsSm, A_PtrBuf, A_StrmSize );
			}
			else
			{
				DbgMsgPrint( "Cls_GrDvrFs3Grp::StreamAdd - none event packet in\n" );
			}
		}
		// check release
		if ( !Tv_Result )
		{
			GrBufTsFree( m_RecBufMng, A_PtrBuf, m_RecOwnerId, m_RecThrdId );
		}
...
//--------------------------------------------------------------------
BOOL8	Cls_GrDvrFs3Grp::LcGthPktAdd( __u8 A_Ch, __u8 A_Type, __u8 A_Evt, __u8 A_Res, __u8 A_SkipCnt, __u8 A_SeqId, 
			Def_GrTime A_TimeSec, __u32 A_TimeMili, BOOL8 A_IsSm, void* A_PtrBuf, __u32	A_StrmSize )
{
	// local -------------------
		BOOL8	Tv_Result;
	...
	// code --------------------
		Tv_Result				=	FALSE;
		Tv_IsNeedAdd		=	FALSE;
		Tv_PktSize			=	sizeof(St_GrDvrAvHd) + A_StrmSize;
		...
		if ( Tv_OccSize < E_GrDvrFs3GrpGthBufSize )
		{
			...
			if ( 0 != m_GthAlloc )
			{
				...
				Tv_IsNeedAdd		=	TRUE;
				Tv_IsNeedBdt		=	FALSE;
				if ( (m_GthAlloc - m_GthInSize) < Tv_OccSize )
				{
					Tv_IsNeedAlloc	=	TRUE;
				}
				if ( E_GrDvrFs3GrpGthFrmCnt <= m_GthInFrm )
				{
					Tv_IsNeedAlloc	=	TRUE;
				}
				if ( 0 != m_GthInSize )
				{
					// check time base over
					if ( E_GrDvrAvTypRsvAdo >= A_Type )
					{
						if ( (m_GthBaseTime > A_TimeSec) || ( (m_GthBaseTime + E_GrTimeSecOfMin) <= A_TimeSec )  )
						{
							// flush
							Tv_IsNeedAlloc	=	TRUE;
							Tv_IsNeedBdt		=	TRUE;
			...
			{
				// build head
				m_GthTbl[m_GthInFrm].AvHd.Fcc			=	E_GrDvrFs3StrmFcc;
				m_GthTbl[m_GthInFrm].AvHd.Ch			=	A_Ch;
				m_GthTbl[m_GthInFrm].AvHd.Type		=	A_Type;
				m_GthTbl[m_GthInFrm].AvHd.IsSm		=	A_IsSm;
				m_GthTbl[m_GthInFrm].AvHd.Evt			=	A_Evt;
				m_GthTbl[m_GthInFrm].AvHd.TimeSec	=	A_TimeSec;
				m_GthTbl[m_GthInFrm].AvHd.DataSize	=	A_StrmSize;
				m_GthTbl[m_GthInFrm].AvHd.TimeMsec	=	A_TimeMili;
				m_GthTbl[m_GthInFrm].AvHd.Res			=	A_Res;
				m_GthTbl[m_GthInFrm].AvHd.SkipCnt	=	A_SkipCnt;
				m_GthTbl[m_GthInFrm].AvHd.SeqId		=	A_SeqId;
				m_GthTbl[m_GthInFrm].AvHd.Wtmk		=	0;
				// mark tail
				Tv_PtrTail	=	(__u32*)( (__u32)A_PtrBuf + Tv_OccSize - sizeof(St_GrDvrAvHd) - E_GrDvrRecTailMarkSize );
...
#endif
				// regist Iov table
				m_GthIov[(m_GthInFrm << 1)].iov_base			=	&m_GthTbl[m_GthInFrm].AvHd;
				m_GthIov[(m_GthInFrm << 1)].iov_len				=	sizeof(St_GrDvrAvHd);
				m_GthIov[(m_GthInFrm << 1) + 1].iov_base	=	A_PtrBuf;
				m_GthIov[(m_GthInFrm << 1) + 1].iov_len		=	Tv_OccSize - sizeof(St_GrDvrAvHd);

				// go next
				m_GthInFrm ++;
				m_GthInSize		=	m_GthInSize + Tv_OccSize;

				// update attribute
				if ( (E_GrDvrAvTypVdoP >= A_Type) || (E_GrDvrAvTypVdoB == A_Type))
				{
					m_GthTmb.AtbTbl[A_Ch]	=	m_GthTmb.AtbTbl[A_Ch] | A_Evt;
				}
...
void	Cls_GrDvrFs3Grp::LcGthBufFlush( BOOL8 A_IsBdtUpdt )
{
	...
	// code --------------------
		...
		{
			if ( m_DiskTbl[m_RecIntIdx]->BlockDataAdd( m_RecBlk, m_RecId, m_GthIov, m_GthInFrm << 1, m_GthInSize , &m_GthTmb, A_IsBdtUpdt ) )
			{
#ifdef E_GrDvrFs3GrpDbgMsgView
				DbgMsgPrint( "%d\n", m_GthIn );
#endif
...
//--------------------------------------------------------------------
void	Cls_GrDvrFs3Grp::LcGthStrmBufFree( void )
{
	// local -------------------
		__u32	Tv_WkIdx;
	// code --------------------
		if ( 0 != m_GthInFrm )
		{
			for (Tv_WkIdx = 0;Tv_WkIdx < m_GthInFrm;Tv_WkIdx++)
			{
				GrBufTsFree( m_RecBufMng, m_GthIov[(Tv_WkIdx << 1) + 1].iov_base, m_RecOwnerId, m_RecThrdId );

				// reset
				/*
				m_GthIov[(Tv_WkIdx << 1)].iov_base			=	NULL;
				m_GthIov[(Tv_WkIdx << 1)].iov_len				=	0;
				m_GthIov[(Tv_WkIdx << 1) + 1].iov_base	=	NULL;
				m_GthIov[(Tv_WkIdx << 1) + 1].iov_len		=	0;
				*/
			}
			DbgMsgPrint( "FS-free %d frm\n", Tv_WkIdx );
		}
