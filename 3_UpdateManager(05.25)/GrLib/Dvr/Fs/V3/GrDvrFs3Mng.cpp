...
//--------------------------------------------------------------------
Cls_GrDvrFs3Mng::Cls_GrDvrFs3Mng( Cls_GrTaskSvr* A_TaskSvr, __u64 A_DvrId, __u32 A_AdoUnitSize,
																 Ptr_GrBufTsMng A_BufRec, __u16 A_RecOwnerMain, __u16 A_RecOwnerMirr, __u16 A_RecThrdId ):
Cls_GrTaskCli()
{
	// local -------------------
	// code --------------------
		...
		m_RecBufMng			=	A_BufRec;
		m_RecOwnerMain	=	A_RecOwnerMain;
		m_RecOwnerMirr	=	A_RecOwnerMirr;
		m_RecBufThrdId	=	A_RecThrdId;
...
//--------------------------------------------------------------------
BOOL8	Cls_GrDvrFs3Mng::StreamAdd( __u8 A_Ch, __u8 A_Type, __u8 A_Evt, __u8 A_Res, __u8 A_SkipCnt, __u8 A_SeqId, 
			Def_GrTime A_TimeSec, __u32 A_TimeMili, BOOL8 A_IsSm, void* A_PtrBuf, __u32	A_StrmSize )
{
	...
	// code --------------------
		...
		{
			if ( m_GrupTbl[E_GrDvrRecDevNoraml]->StreamAdd( A_Ch, A_Type, A_Evt, A_Res, A_SkipCnt, A_SeqId, A_TimeSec, A_TimeMili, A_IsSm, A_PtrBuf, A_StrmSize ) )
			{
				Tv_Result	=	TRUE;
			}
		}
		...
		{
			// release
			GrBufTsFree( m_RecBufMng, A_PtrBuf, m_RecOwnerMain, m_RecBufThrdId );
		}
		// try mirror
		if ( NULL != m_GrupTbl[E_GrDvrRecDevMirror] )
		{
			if ( m_GrupTbl[E_GrDvrRecDevMirror]->StreamAdd( A_Ch, A_Type, A_Evt, A_Res, A_SkipCnt, A_SeqId, A_TimeSec, A_TimeMili, A_IsSm, A_PtrBuf, A_StrmSize ) )
			{
				Tv_Result	=	TRUE;
			}
		}
		...
		{
			// release
			GrBufTsFree( m_RecBufMng, A_PtrBuf, m_RecOwnerMirr, m_RecBufThrdId );
		}
...
void	Cls_GrDvrFs3Mng::LcGroupBuild( void )
{
	...
	// code --------------------
		...
		// create normal group
		m_GrupTbl[E_GrDvrRecDevNoraml]	=	(Cls_GrDvrFs3Grp*) new Cls_GrDvrFs3Grp( this, m_GrupNrmMap, m_IsOvwtNrm, m_IsAutoFix, m_RecBufMng, m_RecOwnerMain, m_RecBufThrdId );

		// create mirror group
		m_GrupTbl[E_GrDvrRecDevMirror]	=	(Cls_GrDvrFs3Grp*) new Cls_GrDvrFs3Grp( this, m_GrupMirMap, m_IsOvwtMir, m_IsAutoFix, m_RecBufMng, m_RecOwnerMirr, m_RecBufThrdId );
