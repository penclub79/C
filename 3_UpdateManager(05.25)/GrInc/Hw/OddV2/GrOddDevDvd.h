...
#ifndef _EPC_GrOddDevDvd
#define _EPC_GrOddDevDvd
...
//====================================================================
//class
class Cls_GrOddDevDvd : public Cls_GrOddDevBase
{
	private:
		__u32 m_NextAddr;
	protected:
		int		log_sense(void* A_PtrScg, caddr_t bp, int cnt, int page, int pc, int pp);
		BOOL	has_log_page(void* A_PtrScg, int page, int pc);
		int		get_log(void* A_PtrScg, caddr_t bp, int *lenp, int page, int pc, int pp);
		void	print_logpages(void* A_PtrScg);
		void	di_to_dstat(struct disk_info *dip, dstat_t *dsp);
		long	rzone_size(track_t *trackp);
	public:
		Cls_GrOddDevDvd();
		virtual	~Cls_GrOddDevDvd();
