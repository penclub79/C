...
//--------------------------------------------------------------------
#ifndef	_EPC_GrP4ConBase
#define	_EPC_GrP4ConBase
...
#define E_GrP4ConPaintMaxCnt			24

#define E_GrP4ConLayerMaxCnt			8
#define E_GrP4ConLayerWinCnt			8

#define E_GrP4ConPntStackCnt			8192

#define E_GrP4ConMseSusTimeOut		50				/* 5 sec */

#define E_GrP4ConPntTxtLineMax		16

#define E_GrP4ConCallbackQuadChg	1					/* callback message quad change */
...
typedef	struct	St_GrP4ConPntStackItm
{
	St_GrRect	Rect;
	__u8	Type;
	__u8	Para0;
	...
	void*	PtrOpt;	
	__s32		IntOpt;	
}	*Ptr_GrP4ConPntStackItm;

typedef	struct	St_GrP4ConPntTxtLine
{
	__s32		SzX;		// x size limit
	__s32		Cnt;		// __s8 count
	__s32		StX;		// base position
}	*Ptr_GrP4ConPntTxtLine;

//====================================================================
//global class
class	Cls_GrP4ConBase
{
	protected:
		...
		Cls_GrP4DoBase*	m_ObjOnMse;	// mouse on object
		Cls_GrP4DoBase*	m_ObjFocus;
		Cls_GrP4DoWin*	m_WinTbl[E_GrP4ConLayerMaxCnt][E_GrP4ConLayerWinCnt];	// window table

		BOOL8					m_IsMseCap;			// mouser capture mode
		BOOL8					m_IsShutDown;

		// quad
		BOOL8	m_IsQuadGridView;
		__u8	m_QuadGridWidth;
		__u8	m_QuadMode;
		__u8	m_QuadPage;
		void*	m_PtrQuadChMap;		// quad channel map
		__u32	m_QuadAbleMap;		// quad mode able map
		St_GrRect	m_QuadRect;		// quad rect on screen
		...
		St_GrRect	m_PntSlotTbl[E_GrP4ConPaintMaxCnt];		// paint slot, if structural element right is zero, this item will not use
		St_GrP4ConPntStackItm	m_PntStack[E_GrP4ConPntStackCnt];
		__u32	m_PntSp;				// paint stack pointer
		Ptr_GrRect	m_PtrPntRect;	// paint working rect

		// paint text setup
		Cls_GrP4Font*	m_PntFont;	// paint font
		...
		St_GrP4ConPntTxtLine	m_PntTxtLineTbl[E_GrP4ConPntTxtLineMax];
		...
		//local function
		void	LcProcUiMsg( Ptr_GrP4MsgHd A_PtrMsg );
		void	LcProcMsgInpMouse( Ptr_GrP4MsgMouse A_PtrMsg );
		void	LcProcMsgInpWheel( Ptr_GrP4MsgWheel A_PtrMsg );
		virtual	void	LcProcTimePeriod( void );
		void	LcProcMsgWinClose( Ptr_GrP4MsgWinClose A_PtrMsg );
		void	LcProcMsgInpKey( Ptr_GrP4MsgKey A_PtrMsg );
		void	LcProcMsgFcsCrt( void );
		void	LcProcMsgLayUpClose( Ptr_GrP4MsgLayer A_PtrMsg );
		virtual	void	LcProcMsgSysStat( Ptr_GrP4MsgSysStat A_PtrMsg );	// system message
	...
	public:
		Cls_GrP4ConBase( BOOL8 A_IsCusSw );
		virtual	~Cls_GrP4ConBase( void );
		...
		Ptr_GrP4ConPntStackItm	PaintStackGetPushPtr( void );						// return = stack pointer
		...
		// object
		Cls_GrP4DoBase*	ObjFindMseByPos( __s32 A_X, __s32 A_Y );
		Cls_GrP4DoBase*	ObjFindByPos( __s32 A_X, __s32 A_Y );
		__u32	ObjListByPos( __s32 A_X, __s32 A_Y, __u32* A_PtrBuf );		// object list by position
		Cls_GrP4DoWin*	WinFindByWinId( __u16 A_WinId );
		Cls_GrP4DoWin*	WinFindByScptId( __u16 A_ScptId );
		...
		virtual	void	QuadGridWidthSet(__u8 A_Width);
		virtual	Ptr_GrRect	QuadGridPosTblPtrGet(__u8* A_PtrRtCnt);
		virtual	Ptr_GrRect	QuadViewRectTblPtrGet(__u8* A_PtrRtCnt);
		__u8*	QuadViewChMapPtrGet( void );
		virtual	__u32	QuadDispChMapGet( void );		// now display channel bit map get
		void	QuadModeGet( __u8* A_PtrRtMode, __u8* A_PtrRtPage );
		virtual	void	QuadRectSet(Ptr_GrRect A_PtrRc);
		virtual	BOOL8	IsQuadGridHw(void);
		virtual	BOOL8	IsQuadCovertHw(void);
		BOOL8	IsQuadGridView(void);
		virtual	void	QuadGridViewSet(BOOL8 A_IsView);
		...
		void	RtlUiMsgProc( Ptr_GrP4MsgHd A_PtrMsg );
		__u16	RtlWinRegist( Cls_GrP4DoWin* A_Win, __u8 A_Layer );		// return = window id
