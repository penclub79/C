...
static AFX_EXTENSION_MODULE RootDllDLL = { NULL, NULL };
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("RootDll.DLL을 초기화하고 있습니다.\n");
		
		// 확장 DLL을 한 번만 초기화합니다.
		if (!AfxInitExtensionModule(RootDllDLL, hInstance))
			return 0;
		...
		new CDynLinkLibrary(RootDllDLL);

	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("RootDll.DLL을 종료하고 있습니다.\n");

		// 소멸자가 호출되기 전에 라이브러리를 종료합니다.
		AfxTermExtensionModule(RootDllDLL);
	}
	return 1;   // 확인
}
