...
#ifndef _EPC_GrOddDevDvdPlusBase
#define _EPC_GrOddDevDvdPlusBase
...
//====================================================================
//class
class Cls_GrOddDevDvdPlusBase : public Cls_GrOddDevBase
{
	protected:
		__u32 m_NextAddr;
		int	ricoh_dvdsetting(void* A_PtrScg, int erase_size, int flags, int immed);
		int	set_p_layerbreak(void* A_PtrScg, long tsize, __u32 lbreak);
		BOOL	dvdplus_ricohbased(void* A_PtrScg);
		void	di_to_dstat(struct disk_info *dip, dstat_t *dsp);
		long	rzone_size(track_t *trackp);
	public:
		Cls_GrOddDevDvdPlusBase();
		virtual	~Cls_GrOddDevDvdPlusBase();
		...
		int	format_unit(void* A_PtrScg,	void *fmt, int length, int list_format, int dgdl,
