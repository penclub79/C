...
#ifndef _EPC_GrOddDevMng
#define _EPC_GrOddDevMng
...
#include	<Hw/OddV2/GrIso9660.h>

//====================================================================
//constance
#define E_GrOddDevMngMaxFileCnt			8
#define E_GrOddDevMngImgHdBufSize		0x0D000
#define	E_GrOddDevMngMaxTrack				99	/* Red Book track limit */

//isoinfo
#define E_GrOddDevMngFileCnt		8
#define E_GrOddDevMngIsoStSec 	0x18
...
#define E_GrOddDevMngErrWriteOver			0
#define E_GrOddDevMngErrWriteErr			1
...
typedef	struct St_GrOddFileInfo 
{
	...
	//Def_StrName	StrFn;		//file name - name only, can not include directory.	
	Def_WstrName StrFn;
...
//====================================================================
//class
class Cls_GrOddDevMng
{
	private:
		Cls_GrOddDevBase* m_PtrDev;
		Cls_GrOddScg*			m_PtrScg;
		Def_StrName			m_StrDev;
		BOOL			m_IsFinish;
		
		__u32		m_Stage;
		//Def_StrName	m_StrVolName;
		...
		__u8		m_ImgHdBuf[E_GrOddDevMngImgHdBufSize];
		...
		track_t*	m_PtrTrack;
		int				m_Tracks;
		track_t		m_Track[E_GrOddDevMngMaxTrack+2];
		long			m_Tsize;
		int				m_Trackno;

		BOOL			m_IsError;
	protected:
		void		LcInitVar( char* A_StrDev );
		BOOL		LcInitDev(__u64 A_IsoSize, BOOL A_IsWrite);
		void		LcReleaseObj( void );

		//iso 2
		void		LcIsoBldTime( void );
		void		LcIsoFillJolietSpace(void* A_PtrBuf, __u32 A_Size);
		__u32		LcIsoStrChr(void* A_PtrBuf, WCHAR* A_Str, BOOL A_IsTypeA, __u32 A_Limite, BOOL A_IsFixSize);
		__u32		LcIsoStrJoliet(void* A_PtrBuf, WCHAR* A_Str);
		__u32		LcIsoDirRecItm(void* A_PtrBuf, __u32 A_SctPos, __u32 A_FileSize, BOOL A_IsDir,
			WCHAR* A_StrFn, BOOL A_IsJoliet);
		__u32		LcIsoDirRecTbl(void* A_PtrBuf, BOOL A_IsSpl);
		...
		BOOL		LcHeadWriteGap( void );
		void		LcHeadBuildImg( void );
		BOOL		LcHeadWriteImg( void );
		BOOL		LcBufWriteData(void* A_PtrData, __u32 A_Size);
		BOOL		LcBufWriteZero(__u32 A_Size);
		BOOL		LcBufFlush(void);

		BOOL		LcPatchWriteMode(void);		//check writeable and mode
	public:
		Cls_GrOddDevMng();
		virtual	~Cls_GrOddDevMng();

		BOOL		Init(char* A_StrDev);

		void		Eject(BOOL A_IsOpen);
		BOOL 		IsExistCdr();

		//normal
		__u64		MediaSizeGet( char* A_StrDev );

		//record
		BOOL		RecFileRegist(char* A_StrFn, __u32 A_Size);
		BOOL		RecPrepare(char* A_StrVol);
		BOOL		RecHeadWrite(void);
		BOOL		RecFileWrite(void* A_PtrBuf, __u32 A_Size, __u8* A_PtrErr);
		BOOL		RecFileZeroWrite(__u32 A_Size);
		BOOL		RecFileClose(void);
		BOOL		RecTailWrite(void);
		BOOL		RecFinish(BOOL A_IsFixate = TRUE);
		__u32		RecFileInfoGet(__u32* A_PtrRtIdx, __u32* A_PtrRtResid);
		__u32		RecStageGet(void);
		BOOL		IsError();

		void		SetStage(__u32 A_Stage);
};
//====================================================================
//global function
