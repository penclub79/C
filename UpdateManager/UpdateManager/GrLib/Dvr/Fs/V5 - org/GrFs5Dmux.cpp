		...
		while(0 < Tv_TryCnt)
		{
			...
			if(m_DmuxDiv.IsOn)
			{
				...
				if(E_GrDvrFrmHeadFcc == Tv_PtrHd->Fcc)
				{
					...
					{
						DbgMsgPrint("Cls_GrFs5Dmux::LcDmuxGoNextFrame - tail divide data size (buf % : frm %) missmatch!\n", m_DmuxDiv.FrmSize, Tv_FrmSize);
					}
#endif
				}
#ifdef E_GrFsDmuxFccChk
				...
				{
					DbgMsgPrint("Cls_GrFs5Dmux::LcDmuxGoNextFrame - tail bad FCC found!\n");
				}
#endif
			}
			else
			{
				...
				if(E_GrDvrFrmHeadFcc == Tv_PtrHd->Fcc)
				{
					...
					if(m_DmuxFrmMaxSize >= Tv_FrmSize)
					{
						...
						if((m_DmuxSctl.Prgs + Tv_FrmSize) <= m_DmuxSctl.InSize)
						{
							...
							{
								DbgMsgPrint("Cls_GrFs5Dmux::LcDmuxGoNextFrame - not match tail (head:%d tail:%d) size!\n", Tv_FrmSize, Tv_TailSize);
							}
#endif
						...
						{
#ifdef E_GrFsDmuxDbgMsg
							DbgMsgPrint("Cls_GrFs5Dmux::LcDmuxGoNextFrame - bad frame size (head:%d tail:%d)!\n", Tv_FrmSize,Tv_TailSize);
#endif
						}
					}
#ifdef E_GrFsDmuxDbgMsg
					...
					{
						DbgMsgPrint("Cls_GrFs5Dmux::LcDmuxGoNextFrame - frame size (%8X) over!\n", Tv_FrmSize);
					}
#endif
				}
#ifdef E_GrFsDmuxFccChk
				...
				{
					DbgMsgPrint("Cls_GrFs5Dmux::LcDmuxGoNextFrame - bad FCC found!\n");
				}
#endif
...
Def_GrErrCode	Cls_GrFs5Dmux::LcDmuxFwNextSctl(void)
{
	...
	// code --------------------
		...
		else
		{
			...
			{
				m_DmuxSctl.Prgs	= Tv_PrvPrgs; //jeff code
			}
		}
