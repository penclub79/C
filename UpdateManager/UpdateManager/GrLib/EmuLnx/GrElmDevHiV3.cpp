/*
	emulation linux HiMPI_V3 device
*/

//====================================================================
//include
#include	<EmuLnx/GrElmDevHiV3.h>
...
//====================================================================
//local const
#define E_GrElmDevHiV3MaxObjCnt		2
...
//====================================================================
//global var
Cls_GrElmDevHiV3*	V_GrElmDevHiV3Obj[E_GrElmDevHiV3MaxObjCnt]	=	
{	NULL, NULL };
file_operations			V_GrElmDevHiV3Fo;

int		V_GrEmlDevHiV3FbNodeTbl[E_GrElmDevHiV3FbCnt]	=	
{ -1, -1, -1, -1, -1, -1, -1 };

//====================================================================
//functions

int GrEmuDevHiV3_ioctl (struct inode * A_PtrInode, struct file * A_PtrFile, DWORD A_Para1, DWORD A_Para2);
int GrEmuDevHiV3_mmap (struct file * A_PtrFile, struct vm_area_struct * A_PtrVm);
int GrEmuDevHiV3_open (struct inode * A_PtrInode, struct file * A_PtrFile);
int GrEmuDevHiV3_release (struct inode * A_PtrInode, struct file * A_PtrFile);

//--------------------------------------------------------------------
Cls_GrElmDevHiV3::Cls_GrElmDevHiV3( DWORD A_DevIdx, BYTE A_FbIdx ):
Cls_GrElmDevBase( A_DevIdx )
{
	...
	// code --------------------
		...
		cdev_init( &Tv_Cdev, &V_GrElmDevHiV3Fo );
		cdev_add( &Tv_Cdev, m_Node, 1 );		// register ioctl

		m_PtrFb	=	malloc( E_GrElmDevHiV3FbSize );

		V_GrElmDevHiV3Obj[m_FbIdx]	=	this;
...
//--------------------------------------------------------------------
Cls_GrElmDevHiV3::~Cls_GrElmDevHiV3()
{
	// local -------------------
	// code --------------------
		if ( NULL != m_PtrFb )
		...
		V_GrElmDevHiV3Obj[m_FbIdx]	=	NULL;
}
//--------------------------------------------------------------------
void*	Cls_GrElmDevHiV3::Mmap( DWORD A_Offset, DWORD A_Size )
{
	// local -------------------
		void*	Tv_Result;
	// code --------------------
		...
		if ( NULL != m_PtrFb )
		{
			if ( (0 != A_Size) && (E_GrElmDevHiV3FbSize >= (A_Offset + A_Size)) )
			{
...
int GrEmuDevHiV3_mmap (struct file * A_PtrFile, struct vm_area_struct * A_PtrVm)
{
	...
	// code --------------------
		...
		if ( NULL != V_GrElmDevHiV3Obj[A_PtrFile->Handle] )
		{
			A_PtrVm->MapAdr	=	V_GrElmDevHiV3Obj[A_PtrFile->Handle]->Mmap( 
...
//--------------------------------------------------------------------
void	Cls_GrElmDevHiV3::LcEmuAvLoad( void )
{
	// local -------------------
		DWORD		Tv_VdoSize;
		Cls_GrFileCtrl*	Tv_Fc;
		Ptr_GrElmDevHiV3FrmHd	Tv_PtrFrm;
		DWORD		Tv_LoadSize;
		DWORD		Tv_Fptr;
		BOOL		Tv_IsStart;
		void*		Tv_PtrData;
	// code --------------------
		...
		if ( Tv_Fc->IsOpened() )
		{
			...
			{
				//check data finished
				if ( (Tv_VdoSize + sizeof(St_GrDvrFrmHdVdo)) >= E_GrElmDevHiV3AvBufSize )
				{
					//finished
					break;
				}
				//load frame
				Tv_PtrFrm		=	(Ptr_GrElmDevHiV3FrmHd)((DWORD)m_EmuVdoBuf + Tv_VdoSize);
				Tv_LoadSize	=	Tv_Fc->Read(Tv_PtrFrm, sizeof(St_GrElmDevHiV3FrmHd));
				if (Tv_LoadSize != sizeof(St_GrElmDevHiV3FrmHd))
				{
					//data ended
					break;
				}
				...
				if ( 0x5641444E == Tv_PtrFrm->Fcc )
				{
					...
					{
						//copy contents
						Tv_PtrData	=	(void*)((DWORD)Tv_PtrFrm + sizeof(St_GrElmDevHiV3FrmHd));
						Tv_LoadSize	=	(Tv_PtrFrm->DataSize + 0x1F) & (~0x1F);
						if ((Tv_VdoSize + sizeof(St_GrElmDevHiV3FrmHd)+ Tv_LoadSize) >= E_GrElmDevHiV3AvBufSize)
						{
							//finished
							break;
						}
						...
						Tv_IsStart	=	TRUE;
						m_EmuVdoCnt ++;
						Tv_VdoSize	=	Tv_VdoSize + Tv_LoadSize + sizeof(St_GrElmDevHiV3FrmHd);
					}
					else
					{
						//skip contents
		...
		{
			DbgMsgPrint( "Cls_GrElmDevHiV3::LcEmuAvLoad - emulation av data open fail.\n" );
		}
		delete	Tv_Fc;
}
//--------------------------------------------------------------------
extern	"C"
{
	HI_S32 HI_MPI_SYS_Init()
	{
		...
		// code --------------------
			...
			// init FO table
			V_GrElmDevHiV3Fo.open			=	GrEmuDevHiV3_open;
			V_GrElmDevHiV3Fo.release	=	GrEmuDevHiV3_release;
			V_GrElmDevHiV3Fo.ioctl		=	GrEmuDevHiV3_ioctl;
			V_GrElmDevHiV3Fo.mmap			=	GrEmuDevHiV3_mmap;

			// register driver 1
			V_GrEmlDevHiV3FbNodeTbl[1]	=	GrElaDrvAdd( "fb1", &V_GrElmDevHiV3Fo );
			if ( 0 <= V_GrEmlDevHiV3FbNodeTbl[1] )
			{
