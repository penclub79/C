...
BOOL CMaskData::IsInputData()
{
	...
	case MaskDataTypeHEXADECIMALupper:
	case MaskDataTypeHEXADECIMALlower:
		bIsInputData = TRUE;
		break;
...
BOOL CMaskData::IsValidInput(TCHAR chNewChar)
{
	...
	case MaskDataTypeHEXADECIMALupper:
	case MaskDataTypeHEXADECIMALlower:
		if ((chNewChar >= _T('A') && chNewChar <= _T('F')) ||
			(chNewChar >= _T('a') && chNewChar <= _T('f')) ||
			(chNewChar >= _T('0') && chNewChar <= _T('9')))
			bIsValidInput = TRUE;
		break;
	case MaskDataTypeCHARACTER:
...
CString COXMaskedEdit::GetMask() const
{
	...
	for (POSITION pos = m_listData.GetHeadPosition(); pos;)
	{
		...
		case MaskDataTypeHEXADECIMALupper:  csMask += chMaskPlaceholderHEXupper; break;
		case MaskDataTypeHEXADECIMALlower:  csMask += chMaskPlaceholderHEXlower; break;
	...
	for (LPCTSTR pszInsertionPoint = pszMask; *pszInsertionPoint; pszInsertionPoint++)
	{
		...
		if (pobjData)
		{
			...
			case chMaskPlaceholderCHARACTER:
				...
				pobjData->m_chValue = m_chPromptSymbol;
				break;
			case chMaskPlaceholderHEXlower: // add HEX decimal mask
				pobjData->m_eType = MaskDataTypeHEXADECIMALlower;
				pobjData->m_chValue = m_chPromptSymbol;
				break;
			case chMaskPlaceholderHEXupper: // add HEX decimal mask
				pobjData->m_eType = MaskDataTypeHEXADECIMALupper;
				pobjData->m_chValue = m_chPromptSymbol;
				break;
